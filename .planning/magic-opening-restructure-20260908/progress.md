...
## 2026-09-08 `1.1.2` CDN 分发收官

- 已快进同步远端 bot 构建提交 `ba1ff77c9abbf140ff622bab41721b6dcc6d7365`。
- 已将 6 个脚本加载地址从 `127.0.0.1:5510` 改为该固定 CDN 提交，并将卡版本提升为 `1.1.2`。
- `pnpm build` 与 `node tavern_sync.mjs bundle 魔法禁书目录模拟器` 成功；构建钩子产生的无关 dist 时间戳噪音已恢复。
- 最终 PNG：`src/魔法禁书目录模拟器/魔法禁书目录模拟器.png`，7,320,821 bytes。
- 最终 SHA256：`708204B857813F7F26FC4194434999FFA320D93C2DB06E5C1FBE0D818FAD07AC`。
- 静态断言通过：`chara` / `ccv3` 均为版本 `1.1.2`，本地 `5510` 残留 0，固定 CDN 引用 6。
- 用户已重新导入 `1.1.2`，真机只读验收通过：SillyTavern `1.18.0`、Tavern Helper `4.9.3`、世界书 114 条、正则 7 条、4 层、106 组、114 Opening、114 组兼容/元数据载荷。
- 6 个固定 CDN 脚本全部 HTTP `200`；`Mvu` 全局存在；自定义开场控件保留；无 `创约15`。
- 本轮未升级外部依赖版本，未点击开场或改写聊天变量；完整交互链路可后续单独回归。
