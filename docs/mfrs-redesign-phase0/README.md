...
**一句话**：请重导 **8.13.15**（initvar 四键升根 + schema 扩行动建议/判定字段）。
...
- 当前发布版：**8.13.15**（`…-v81315-bf0-initvar-schema` / `@107b3ff`）  
  PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`  
- 回滚：8.13.14 `@d5cd98f…`（C7 always-unlock，无 BF0 契约）
