...
> **历史阶段档案**：本文记录 8.11–8.13.36 的设计与发布过程，不代表当前项目运行流程或当前发布版本。现行流程见根目录 `PROJECT_FLOW.md`，当前版本见 `scripts/mfrs-release-constants.mjs` 与 `task_plan.md`。
...
| **8.13.23–8.13.29** 后续维护与沉浸式按键审查修复 | **已发**；该阶段内容版本为 **8.13.29**（release `410454b`，CDN dist `95981c9`） |
| **MAINT-29** 黄金储备正式路径 + drawer watcher 生命周期 | **已发** → release `4c94a4e`，tag `v8.13.31` |
| **8.13.36** 沉浸 HUD 中栏档案预览、记忆 CRUD 与抽卡嵌入 | **已发** → [RELEASE_8.13.36.md](./RELEASE_8.13.36.md) |

**一句话**：8.13.36 已交付沉浸 HUD 中栏工作区（档案预览、记忆 CRUD、抽卡嵌入与删除安全门禁）。
...
- 该阶段最终发布内容：**8.13.36**（CDN dist `9c5a467a348`；cache `v81336_20260716_01`；release commit/tag 见 [RELEASE_8.13.36.md](./RELEASE_8.13.36.md)）
  - PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`
- 历史回滚点：8.13.31 `@8ee8c584fc60`；8.13.29 `@95981c90c901`；8.13.22 `@158dcc29107f`；更近版本按 git tag/release 记录选择
