...
**一句话**：请重导 **8.13.10**（MVU 未落库时仍可从 AI 的 UpdateVariable 显示本轮选项）。
...
- 当前发布版：**8.13.10**（`…-v81310-actions-updatevar-fallback`）  
  PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`  
- 回滚：8.13.9 `@55adbd8…`（无 UpdateVariable 回退时选项可能空白）
