...
- [ ] **T3.1 删除指定入口**：从 `buildHudDossierHtml()` 删除“打开全库 · 玩家状态”。
  - 文件：`消息内面板/index.ts`
  - 完成条件：只删按钮；玩家状态数据、镜像、通用表导航和系统全库入口仍在。
- [ ] **T3.2 默认模式增加入口**：在默认最新 AI 三栏的 7 键导航下增加独立“沉浸模式”按钮。
  - 完成条件：使用展开图标、可见焦点、明确 `aria-label`；不计入 7 个业务视图。
- [ ] **T3.3 接入现有切换生命周期**：默认按钮调用 `toggleHudImmersive()`，不新增模式状态真源。
  - 依赖：T3.2
  - 完成条件：继续使用 `hudImmersivePreferred`；不写 localStorage；`Ctrl+Shift+G` 保留。
- [ ] **T3.4 沉浸反向入口**：顶栏“退出沉浸”改为图标 + “默认模式”，仍调用 `exitHudImmersive()`。
  - 完成条件：按钮语义与默认入口成对；首次自动沉浸策略不变。
- [ ] **T3.5 焦点与响应式**：补默认按钮刷新焦点恢复和桌面/移动稳定尺寸。
  - 依赖：T3.2–T3.4
