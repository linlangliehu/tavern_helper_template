...
## T7：全量验证与发布 — **静态/聚合/发布/发布后 complete; 真页 T7.3/T7.4 pending**
...
- [x] **T7.2 专项与聚合门禁**（`verify:mfrs-source-gates` 阶段1 + `verify:mfrs-gates` 阶段2 + 双卡身份门禁，8.14.15 通过）
  ...
  - 双 YAML/双 PNG身份门禁
- [ ] **T7.3 真页桌面验收**  ← 仍 pending：8.14.15 发布未见本轮桌面真页回归记录
  - 普通人、单鬼、双鬼开局。
  - 重载 5 次、换聊、切卡、pagehide、等待 25 秒。
  - 复苏 99→100 的普通与豁免路径。
- [ ] **T7.4 真页移动端验收**  ← 仍 pending：8.14.15 发布未见 390px 移动端回归记录
...
- 仍 pending 的 4 项：**T0.1**（基线证据保存，8.14.15 发布后仅剩归档意义）、**T6.3**（干净角色列表重新导入验收）、**T7.3**（桌面端真页回归）、**T7.4**（移动端真页回归）。
