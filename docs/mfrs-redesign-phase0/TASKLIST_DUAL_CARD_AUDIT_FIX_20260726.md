...
## T7：全量验证与发布 — **静态/聚合/发布/发布后/桌面真页/移动端真页 complete; 复苏终局需用户手动**
...
- [x] **T7.4 真页移动端验收**（2026-08-14 v8.15.14 开发版 Chrome DevTools emulate 390x844）
  - 默认模式：CSS 媒体查询 ≤640px 生效 ✓（grid-template-areas: "right" "center" "left"）
  - 导航按钮 7 个全部可见、两行 wrap 布局、无溢出 ✓（正文/档案/关系/记忆 54x44，抽卡/系统/设置 74x44）
  - 沉浸模式按钮 235x44 可见 ✓
  - 开局表单 170px 宽无溢出 ✓（4 select + 5 textarea + 提交按钮全部在视口内）
  - 沉浸模式：Ctrl+Shift+G 正确触发 ✓；左右栏 display=none ✓；顶栏 390x40 ✓；退出按钮 94x44 在 x=288 可见 ✓
  - 沉浸→默认切换：body scrollWidth=390=viewport，无溢出 ✓
  - 控制台无新增 error（仅 vendor 非关键 404）
...
- 仍 pending 的 2 项：**T0.1**（基线证据保存，8.14.15 发布后仅剩归档意义）、**T6.3**（干净角色列表重新导入验收）。
...
- **2026-08-14 T7.4 移动端 390px 真页回归已完成**（v8.15.14 开发版 Chrome DevTools emulate 390x844）：默认模式导航 7 键全部可见无溢出 ✓、开局表单无溢出 ✓、沉浸模式 Ctrl+Shift+G 触发+退出按钮可见 ✓、沉浸↔默认切换无溢出 ✓。
- **后续 8.15.0（2026-08-12）补充真页验收**：人物/地点 stat_data 镜像链路在开发版做了一次真页回归（GENERATION_ENDED → mvu-core-mirror → 写库，只补不覆盖），但该验收聚焦镜像功能。T7.3/T7.4 已于 2026-08-14 补充完成。
