...
- 已完成发布 PNG 与固定 CDN bundle 审阅；确认发布 bundle 同样含状态栏生命周期缺陷。
- `verify:mfrs-dist-freshness` 按预期检出 pin 与 HEAD dist 不一致并 exit 1；已记录为可复现性风险，不夸大为功能漏发。
...
- 完成：入口/381 文件引用、脚本生命周期、变量/提示词/开局契约、CDN pin 与现有聚合门禁均已复核。
- 验证：`pnpm verify:mfrs-gates` 7/7 通过；附带警告 CDN_REF 落后 HEAD 21 提交（11 个非 bundle）。
- 只修改三份规划记录；业务源码未改，用户原有未跟踪 `酒馆助手脚本-tavern_sync.json` 未触碰。
