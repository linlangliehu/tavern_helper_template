...
**2026-07-07 v8.5.12 远端发布 smoke 完成：修复第二轮对话缺消息内状态面板。** 当前导入卡 `@15c2feb ... mvu-v8511` 中，#4 第二轮 AI 回复没有 `.mfrs-msg-panel`，但 `chat[4].variables[0].stat_data` 与 `Mvu.getMvuData({type:'message',message_id:4})` 一致，行动建议长度为 4；手动调用 `MysteryMessagePanel.refreshMessage(4)` 后面板立即出现，证明不是 MVU/EJS/字段映射失败，而是消息内面板自动补渲染漏跑。已在 `src/神秘复苏模拟器/脚本/消息内面板/index.ts` 增加 `GENERATION_ENDED/GENERATION_STOPPED` 后多段延迟刷新，以及 `MutationObserver` 监听最终 `.mes/.mes_text` 替换后补跑 `processAllMessages()`；回归脚本已加静态 gate。source commit `6b77048` 已 push，bot bundle `8b3ea67 [bot] bundle` / tag `v0.0.387` 已生成，发布同步 commit `0eafa0e chore(release): publish mfrs v8.5.12` 已 push。验证通过：`node --check scripts\verify-mfrs-mvu-hotfix-regressions.mjs`、`pnpm verify:mfrs-mvu-hotfix`、`git diff --check`、`pnpm build`、worldbook gate 383/33/5851、YAML/PNG `8.5.12` + `8b3ea67` + `mvu-v8512` 且旧 ref/cache/local 均 0、CDN smoke `@8b3ea67` 消息内面板/hotfix/数据库 loader 均 200；远端 `@0eafa0e` 发布版 YAML/PNG HTTP 200，远端 PNG worldbook gate 383/33/5851，`chara/ccv3` marker 均正确。未发送消息、未触发真实 AI、未点击“立即手动更新”、未调用 `manualUpdate()` / `triggerUpdate()`。当前任务收口。
