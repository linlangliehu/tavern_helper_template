...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（版本 8.4.5，CDN `@ec3a312`）
- 开发版源码版本：`2.0`（开发版 yaml 版本号，与发布版独立）；v8.4.5 发布版已生成并推送，用户导入新版 PNG 后生效。
...
+ v8.4.3 CHAT_CHANGED 原生事件回退（`294cc1a`）+ v8.4.4 新建聊天 CHAT_CHANGED 轮询等待数据切换（`491fe43`）+ v8.4.5 货币监听器跳过开场白/静默生成（`73b77aa`）。
...
**下次恢复入口更新为：** 读 progress.md 顶部 "v8.4.5 发布：货币监听器跳过开场白/静默生成" 条目。用户重新导入 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 即可获得 v8.4.5 修复（开局不再误发调查点）。真页验证修复需导入 v8.4.5 后新建聊天确认不再弹"获得调查点"toast。v8.4.4 真页验证已通过（新建聊天自动恢复 14 表）。

**工作区状态：** 当前主线已包含 v8.4.1 source commit `6cb397f`、bot bundle `a34b4d5` 和发布同步 commit `8b2d759`。主工作区仍可能有本地 `dist/**` 构建残留，以及既有无关 dirty：`.claude/worktrees/*`、`.tmp-jerryzmtz-my-tavern-scripts/` — 不要提交。
...
**当前无待提交发布同步文件（2026-06-29 v8.4.1 已 push）：** v8.4.1 发布同步提交为 `8b2d759`，包含 `scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器.png`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`。源码修复已在 `6cb397f`，dist 已由 bot bundle `a34b4d5` 重建；不要提交主工作区本地 `dist/**` 构建残留。
**当前无待提交发布同步文件（2026-06-30 v8.4.5 已 push）：** v8.4.5 发布同步提交为 `005d4ec`，包含 `scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`。源码修复已在 `73b77aa`（merge `bb5c5fb`），dist 已由 bot bundle `ec3a312` 重建；不要提交主工作区本地 `dist/**` 构建残留。
