...
**当前版本：**
- 最新发布包为 v8.5.7：自动剧情/记忆召回已进入发布版；source `f67c780`，bot bundle `bbbe6c7`（tag `v0.0.371`）作为 CDN_REF；releaseVersion=8.5.7；发布版 YAML/PNG 已生成并验证，发布同步 commit `27acf1f`，planning 收口 commit `0242af8`。
...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（当前发布版本 8.5.7，CDN `@bbbe6c7`）
...
5. ⏳ 走正式链路：source commit/push → 等 bot bundle → 回填 `scripts/publish-card.mjs` 为新 CDN ref 和版本候选 `8.5.8` → `pnpm run publish-card -- 神秘复苏模拟器发布版` → YAML/PNG/CDN smoke。
...
**当前待提交候选（2026-07-07 发布后真实对话问题修复，任务 1-5 已实施，P5 发布链路未执行）：**
- 协议/变量源码：`src/神秘复苏模拟器/世界书/变量/变量输出格式.yaml`、`src/神秘复苏模拟器/世界书/变量/initvar.yaml`、`src/神秘复苏模拟器/世界书/变量/变量列表.txt`，以及发布版同名文件。
- 协议描述与示例：开发版/发布版 `系统提示词/0.txt`、`对话示例/0.txt`、`世界书/规则/事件MVU联动规则.txt`。
- 兼容解析：`src/神秘复苏模拟器/界面/状态栏/App.vue`、`vendor/shujuku-sp-fork/index.js`、`scripts/verify-output-cleaning-regressions.mjs`。
- 构建/打包产物：`pnpm build` 已刷新开发版与发布版 PNG；是否在 P5 发布前提交 PNG 需按发布策略决定。`dist/**` 仍默认不提交，交给 bot bundle。
- 若正式发布同步：再更新 `scripts/publish-card.mjs`、运行 `pnpm run publish-card -- 神秘复苏模拟器发布版` 并提交发布版 `index.yaml` / PNG。
- planning 记录：`task_plan.md`、`progress.md`、`findings.md`。
- 默认仍不提交 `dist/**`、`.tmp-mfrs-regex-backup-20260707.json`、临时探针 JSON、CDP 日志或未跟踪截图 `屏幕截图 2026-07-06 235029.png`。
