...
**2026-07-08 v8.5.14 远端发布完成：跨角色卡污染清理。** 本轮起因是会话恢复时在 `task_plan.md` 标记"无未决项"但工作区发现一批已存在但未在册的功能改动（既有未提交源码、又有改动对应回归脚本），经 `git diff` 定位为「切到非神秘复苏卡时主动 cleanup 留在主窗口的 DOM/globals/resource URLs，切回本卡时正常重挂」。立项 v8.5.14 后完成全链路：固定状态栏新增 `isMysteryRevivalCardActive()` / `cleanupFixedStatusBar()` / `handleChatChanged()` / `installCleanup()`；数据库前端拆 `cleanupMfrsDatabaseFrontend()` / `installDatabaseFrontendCleanup()` / `loadAcuFrontendRuntime()` 按需 eagerly import；`v10_2_visualizer.js` 暴露 `MysteryAcuVisualizer.cleanup`（断开 Observer + `unregisterTableUpdateCallback` + 移 ACU 样式/DOM）。同步更新两个回归脚本静态 gate（旧 `index.ts` 静态 import frontend-config/visualizer 顺序断言改为 `loadAcuFrontendRuntime` runtime-loader 断言）。真页非 AI smoke 在 Science Worship 20260628 卡上验证 cleanup 路径完全有效：before `MFRS/MysteryDatabaseFrontend/MysteryAcuVisualizer=object` + 3 个 style 节点 + 2 个残留 `__mfrsScriptResourceUrls__` 键，after 全部 `undefined` / 节点移除 / markers 清空，`Mvu` 框架本身保留预期。source `b53f5b5` → bot bundle `0717fc4`（tag `v0.0.391`）→ publish sync `7a52ae9` 已 push origin/main。publish-card 配置 `CDN_REF=0717fc4` / `releaseVersion=8.5.14` / `cache=phase164-...-mvu-v8514`。验证通过：YAML 8.5.14×1 + 0717fc4×7 + mvu-v8514×8 + 旧 8.5.13/8b3ea67/mvu-v8513/local/`@main`/`@52b2e62` 全 0；PNG chara/ccv3 各同 marker；worldbook gate 383/33/5851；CDN `@0717fc4` 4 个 mfrs bundle HTTP 200 含 v8.5.14 cleanup marker；远端 `@7a52ae9` YAML/PNG 200 同 marker 全绿。未触发真实 AI、未发送消息、未点击"立即手动更新"、未调 `manualUpdate()` / `triggerUpdate()`。待用户导入 v8.5.14 后真实使用观察跨卡行为。

**2026-07-08 v8.5.13 远端发布完成并经用户真实对话复测通过：修复「关系/环境」在场人物恒为"暂无在场人物"。** 用户导入卡并真实对话后反馈消息内面板在场人物一直空。Chrome DevTools MCP 只读排查确认这不是渲染 bug：`消息内面板/index.ts:166` 读 `data.在场人物`，当时真页最新楼层 #4 的 `Mvu.getMvuData({message_id:4}).stat_data.在场人物 === []`、`chat[4].variables[0].stat_data.在场人物 === []`（字段在、类型 Array、但空），面板如实显示"暂无"。#2/#4 的 `extra._mfrs_raw_protocol_message` 原始协议搜 `在场人物` 0 命中；#4 实际 JSONPatch 路径只有 `/风险值`、`/当前灵异事件/*`、`/主线进度/*`、`/规律推理记录/-`、`/最近行动判定`、`/行动建议`，从没 `/在场人物`。根因是 `src/神秘复苏模拟器{,发布版}/世界书/变量/变量输出格式.yaml` 的 `update_output_contract` 从未要求 AI 更新 `/在场人物`，schema (`schema.ts:182`) 与 `initvar.yaml:31` 默认 `[]`，AI 不写就恒空。已在两份契约的示例 patch 与 `patch_rules` 补 `/在场人物` 规则（"名字-简述"字符串数组，NPC 出场/离开/死亡/揭示时 `replace`，玩家独处用 `[]`，面板 `index.ts:172` 已支持"名字-描述"拆分着色）。纯世界书改动、无 dist 变更，CDN_REF 沿用 `8b3ea67`（同 v8.4.6 先例），releaseVersion 8.5.12→8.5.13、cache `mvu-v8513`。验证通过：YAML parse、`node scripts/verify-output-cleaning-regressions.mjs`、`node --check scripts/publish-card.mjs`、`git diff --check`、worldbook gate 383/33/5851、发布版 PNG chara/ccv3 各含 `/在场人物` 规则+示例 patch、8.5.13×1、`8b3ea67`×7、`mvu-v8513`×8、旧 8.5.12/mvu-v8512/localhost 均 0。单 commit `e6b9ebe fix(mfrs): populate 在场人物 via output contract (v8.5.13)` 已 push origin/main（`ffa5bab..e6b9ebe`）。复测时用户已重新导入 v8.5.13 并真实对话；DevTools 只读确认完整角色对象含 8.5.13×2、`@8b3ea67`×14、`mvu-v8513`×16，旧 8.5.12/mvu-v8512 为 0；AI 楼层 #2/#4 原始 JSONPatch 均含 `/在场人物`，`chat[i].variables[0].stat_data` 与 `Mvu.getMvuData()` 均为非空数组，关系/环境 tab 不再显示"暂无在场人物"。本次验证未触发真实 AI、未发送消息、未点击"立即手动更新"、未调用 `manualUpdate()` / `triggerUpdate()`。修复确认生效。
...
**历史状态快照（v8.5.11 及更早）已归档：** 逐版本收口详情见下方「版本变更索引」表和 progress.md 对应条目，不再在此重复散文快照。

---

**当前版本：**
- **最新发布包为 v8.5.14**：跨角色卡污染清理并发布完成；source `b53f5b5`，bot bundle `0717fc4`（tag `v0.0.391`），publish sync `7a52ae9`；CDN_REF=`0717fc4`/releaseVersion=8.5.14/cache=`mvu-v8514`。
...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（当前已发布版本 8.5.14，CDN `@0717fc4`，cache `mvu-v8514`）
...
**下次恢复入口（当前）：** ✅ **v8.5.14 已发布完成并远端 smoke 通过（跨角色卡污染清理）**。新对话先读本文件顶部“当前状态”“当前任务清单”“版本变更索引”、`PROJECT_FLOW.md`、`progress.md` 顶部最近 2-3 条、`findings.md` 顶部相关经验。当前 `origin/main = 7a52ae9 chore(release): publish mfrs v8.5.14`，本机 main 已同步。当前发布 PNG 为 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`，版本 `8.5.14`，CDN `@0717fc4`，cache `mvu-v8514`，source commit `b53f5b5`，bot bundle `0717fc4`（tag `v0.0.391`），publish sync `7a52ae9`。当前任务已收口，无未决开发项；待用户导入 v8.5.14 后真实使用观察跨卡行为（切非神秘复苏卡确认无 `MFRS/MysteryDatabaseFrontend/MysteryAcuVisualizer/mfrs-fixed-status-host/.acu-*` 残留；切回神秘复苏卡确认功能正常）。`.tmp-mfrs-regex-backup-20260707.json`、`.tmp-chrome-mfrs-validation/` 与未跟踪截图仍不提交。
**历史恢复入口（已折叠）：** v8.5.11/v8.5.10/v8.5.1/v8.5.0/v8.4.9/v8.4.7-8/v8.4.1 等旧版本的逐版本恢复入口详情，见「版本变更索引」表与 progress.md 对应条目，不再在此逐条重复。

**工作区状态：** 当前主线已包含 v8.5.13 单 commit `e6b9ebe`（在场人物契约 source + 发布同步合并）；仅有本地参考文件 `.tmp-mfrs-regex-backup-20260707.json` 和未跟踪截图 `屏幕截图 2026-07-06 235029.png`，以及 `dist/神秘复苏模拟器/**` 本地 dev build 噪声，默认均不提交；后续新任务仍需精确 staging。

## 当前任务清单

**当前无未决开发项。** 最新发布版 v8.5.14（跨角色卡污染清理）已 push origin/main（source `b53f5b5` → bot bundle `0717fc4` → publish sync `7a52ae9`），远端 YAML/PNG smoke 与本地真页非 AI smoke 均通过。待用户导入 v8.5.14 后观察跨卡实际行为：切非神秘复苏卡时 `MFRS`/`MysteryDatabaseFrontend`/`MysteryAcuVisualizer`/`mfrs-fixed-status-host`/`acu-*` 全部消失；切回神秘复苏卡时固定状态栏/数据库前端/召回 tab/抽卡面板/消息内面板等正常重挂。

**已完成任务清单（勿重做）已折叠：** v8.5.13 在场人物输出契约修复；v8.5.9/v8.5.10 真实对话收口 P0–P9；发布后真实对话修复 P0–P5；自动剧情/记忆召回任务 1–7；数据库前端 P0–P3/总览-一致性-抽卡增强；抽卡系统 9 任务；四优先级改进（v7.6~v8.0）等均已 100% complete。逐项收口详情见下方「版本变更索引」表与 progress.md 对应条目；需要回查旧任务分解时看 progress.md 或 `planning_archive_2026-06/`，不要从旧清单的"待合并/待发布"描述恢复任务。
