...
**2026-07-07 v8.5.12 发布同步完成：修复第二轮对话缺消息内状态面板。** 当前导入卡 `@15c2feb ... mvu-v8511` 中，#4 第二轮 AI 回复没有 `.mfrs-msg-panel`，但 `chat[4].variables[0].stat_data` 与 `Mvu.getMvuData({type:'message',message_id:4})` 一致，行动建议长度为 4；手动调用 `MysteryMessagePanel.refreshMessage(4)` 后面板立即出现，证明不是 MVU/EJS/字段映射失败，而是消息内面板自动补渲染漏跑。已在 `src/神秘复苏模拟器/脚本/消息内面板/index.ts` 增加 `GENERATION_ENDED/GENERATION_STOPPED` 后多段延迟刷新，以及 `MutationObserver` 监听最终 `.mes/.mes_text` 替换后补跑 `processAllMessages()`；回归脚本已加静态 gate。source commit `6b77048` 已 push，bot bundle `8b3ea67 [bot] bundle` / tag `v0.0.387` 已生成，`scripts/publish-card.mjs` 已回填 `CDN_REF=8b3ea67`、`CDN_CACHE_VERSION=phase164-4-0-final-baseline-6-28-p5-4-hotfix13-mvu-v8512`、`releaseVersion=8.5.12` 并重新生成发布版 YAML/PNG。验证通过：`node --check scripts\verify-mfrs-mvu-hotfix-regressions.mjs`、`pnpm verify:mfrs-mvu-hotfix`、`git diff --check`、`pnpm build`、worldbook gate 383/33/5851、YAML/PNG `8.5.12` + `8b3ea67` + `mvu-v8512` 且旧 ref/cache/local 均 0、CDN smoke `@8b3ea67` 消息内面板/hotfix/数据库 loader 均 200。未发送消息、未触发真实 AI、未点击“立即手动更新”、未调用 `manualUpdate()` / `triggerUpdate()`。下一步：提交并推送 v8.5.12 发布同步。

**2026-07-07 v8.5.11 当前导入卡真页验证通过，远端发布 smoke 完成。** 用户重新导入并对话后，本轮用 Chrome DevTools MCP 只读验证当前页：角色 `神秘复苏模拟器发布版`，脚本资源已是 `@15c2feb ... mvu-v8511`，运行态 `Mvu.*`、`TavernHelper.getVariables/updateVariablesWith`、`ctx.saveChat` 均存在。最新 AI 楼层 #2 的 `chat[2].variables[0].stat_data` 与 `Mvu.getMvuData({type:'message',message_id:2})` 一致，已写入 `姓名=次卧室`、`身份=初级驭鬼者`、`所在位置=大昌市第七中学教室`、`事件代号=七中灵异事件`、`行动建议.length=4`，证明 v8.5.10 的 MVU 写回与持久化修复已生效。消息内面板 DOM 已显示 A/B/C/D 的 `思路` 与风险/收益摘要，`domHasUnknownAction=false`、`.mfrs-msg-action-meta=4`，证明 v8.5.11 显示修复在当前导入卡中生效。“未知行动”不是 MVU 写回或 EJS 问题，而是旧 `消息内面板` 显示层按旧字段 `label/text` 读取行动建议；当前变量/检定建议结构使用中文字段 `选项/思路/主要风险/预期收益/死亡风险/复苏风险`。已在 `src/神秘复苏模拟器/脚本/消息内面板/index.ts` 修复为优先读取 `选项/思路` 并显示风险/收益摘要，source commit `157917a` 已 push，bot bundle `15c2feb [bot] bundle` / tag `v0.0.384` 已生成，发布同步 commit `93fe875 chore(release): publish mfrs v8.5.11` 已 push。验证通过：hotfix 回归、`pnpm build`、worldbook gate 383/33/5851、YAML/PNG `8.5.11` + `15c2feb` + `mvu-v8511` 且旧 ref/cache/local 均 0、CDN smoke `@15c2feb` 消息内面板/hotfix/数据库 loader 均 200 且无 stale ref；远端 `@93fe875` 发布版 YAML/PNG HTTP 200，远端 PNG worldbook gate 383/33/5851，`chara/ccv3` marker 均正确。未发送消息、未触发真实 AI、未点击“立即手动更新”、未调用 `manualUpdate()` / `triggerUpdate()`。

**2026-07-07 v8.5.10 远端发布完成：verified writeback + persistence 已进入发布版 PNG。** 本轮用 CDP 真页探针和源码复核确认 v8.5.9 失败不在发布包、vendor 链、nested `<JSONPatch>`、raw protocol 保存、`Mvu.parseMessage()` 或 `Mvu.replaceMvuData()` API 本身；真正缺口是 hotfix 单次 `replaceMvuData` 后信任日志，没有写后读回校验、直接变量兜底、事件链稳定后的延迟重试、安装恢复补写，并且真页 `getVariables/updateVariablesWith` 位于 `window.TavernHelper`，直接兜底写内存后需要 `SillyTavern.getContext().saveChat()` 持久化。角色卡变量链路已确认：`initvar.yaml` 顶层对齐 schema；`变量列表.txt` 是只读 EJS，读 `variables.stat_data` 并输出 `<stat_data>`；消息内面板按 DOM `mesid` 读取 `getVariables({type:'message',message_id}).stat_data`；数据库前端只读最新 MVU/数据库镜像，不证明历史 message variables 正确。source commit `1a3c660 fix(mfrs): persist verified mvu writeback` 已 push；bot bundle `c576add [bot] bundle` / tag `v0.0.381` 已生成并验证 dist 命中 `TavernHelper/saveChat/persisted` 与 direct `chat.variables[...]` 写回逻辑；发布同步提交 `00ac021 chore(release): publish mfrs v8.5.10` 已 push。验证通过：hotfix 回归、`pnpm build`、worldbook gate 383/33/5851、YAML/PNG `8.5.10` + `c576add` + `mvu-v8510` 且旧 ref/cache/local 均 0、CDN smoke `@c576add` hotfix/数据库 loader/数据库前端/消息内面板均 200 且无 `@main/@52b2e62/file:///`、远端 `@00ac021` 发布版 YAML/PNG HTTP 200，远端 PNG worldbook gate 383/33/5851，`chara/ccv3` marker 均正确。未发送消息、未触发真实 AI、未点击“立即手动更新”、未调用 `manualUpdate()` / `triggerUpdate()`。下一步：用户重新导入/更新到 v8.5.10 后可做一次真实对话复测。

**2026-07-07 v8.5.9 用户真实对话复测失败：发布包生效，但 MVU message variables 仍未写入。** 用户已导入 `神秘复苏模拟器发布版` 并进行了数轮真实对话；本轮 Chrome DevTools MCP 只读验证确认当前角色为 `神秘复苏模拟器发布版`、chat length 5、AI 消息为 #0/#2/#4，运行态存在 `window.Mvu`、`MysteryMessagePanel.refreshMessage`、`window.__mfrsScriptResourceUrls__` 指向 `@e36f8aa ... mvu-v859`。第 #2/#4 条 AI 回复 visible `mes` 已被清洗不含 `<UpdateVariable>`，但 `extra._mfrs_raw_protocol_message` 保存了原始协议，且 nested `<JSONPatch>` 可正常解析：#2 有 16 个 `replace`，#4 有 14 个 `replace`、2 个 `delta`、1 个 `insert`，包含 `/姓名`、`/身份`、`/所在位置`、`/当前灵异事件...`、`/行动建议` 等关键路径。失败点是写回后运行态仍为初始值：`chat[2].variables[0].stat_data.姓名 === "未知"`、`chat[4].variables[0].stat_data.姓名 === "未知"`、`当前灵异事件.事件代号 === "未立案灵异事件"`、`行动建议.length === 0`，`.mfrs-msg-panel` 也显示 `未知 / 未立案灵异事件 / 暂无行动建议`；底部数据库仪表盘却显示 `测试 / 初级驭鬼者 / 大昌市第七中学教室内 / 最近行动...`，说明数据库镜像路径成功、MVU/message variables 路径失败。console 关键证据：`[Hotfix] GENERATION_ENDED 触发 {"messageIndex":4,...,"hasUpdateVariable":true}`，随后出现 `mag_variable_update_started`/`mag_command_parsed`/`mag_variable_update_ended`，并打印 `[Hotfix] MVU parseMessage 已解析并写回消息变量 {"writer":"Mvu.replaceMvuData"}`；但紧接着 Prompt Template 渲染 `message #4.0 variables` 仍是初始 `stat_data`。当前结论：v8.5.9 没有完全修复真实对话问题；下一步应定位 `Mvu.replaceMvuData` 的写回目标/数据形态为何没有落到 `chat[i].variables[0]` 或 TavernHelper 实际读取层。

**2026-07-07 v8.5.8 真实对话失败二次修复 P0-P7 已完成并发布为 v8.5.9。** 本轮已修 `hotfix-generation-ended-listeners`：不再把 message index 传给 `Mvu.parseMessage()`，而是保存清洗前原始协议、把旧 `<UpdateVariable>[...]</UpdateVariable>` 归一化为 nested `<JSONPatch>`，并把 `op:"add"` 安全转换为 `replace` / `insert` 后写回 message MVU data；消息内面板新增 `MysteryMessagePanel.refreshMessage()`，hotfix 写回后可刷新单条面板。首轮契约已强化为完整 `<UpdateVariable><JSONPatch>...</JSONPatch></UpdateVariable>` 骨架，变量输出规则禁止 direct-array 与 `op:"add"`；`[mvu_update]变量更新规则` 改为蓝灯常驻。P4 曾发现 `import.meta.url` 会被 webpack 固化成 `file:///D:/.../index.ts` 并退到 `@main/vendor`，现已改为由卡内 wrapper 写入 `window.__mfrsScriptResourceUrls__`，bundle 再从运行时实际 URL / performance entries / stack 推导同 bundle 根路径；dist 回归禁止 `file:///`、`@main/`、旧 `@52b2e62`。P4 no-AI 运行态 smoke 已证明本地 loader 请求 `127.0.0.1:5501/dist/.../数据库/index.js` 后，实际加载同根 `/vendor/shujuku-sp-fork/index.js?...mvu-v859...`。P6 真页 no-AI smoke 已通过：旧 direct-array + `op:"add"` 样本归一化后更新 `姓名=P6可逆测试`、`风险值=5`、`当前灵异事件.事件代号=P6敲门鬼媒介传播事件`、`鬼域状态=疑似鬼域`、`行动建议.length=1`，消息内面板 DOM 同步显示临时值，随后恢复原变量且无测试残留；未发送消息、未触发真实 AI、未点击“立即手动更新”、未调用 `manualUpdate()` / `triggerUpdate()`。source commit `5f38953` 已 push，bot bundle `e36f8aa [bot] bundle` / tag `v0.0.378` 已生成，发布同步 commit `ec6b64a chore(release): publish mfrs v8.5.9` 已 push；发布版 YAML/PNG 与 CDN smoke 均确认 `8.5.9`、`e36f8aa`、`mvu-v859` 生效，无旧 `454267e` / 8.5.8 / local / `@main` / `@52b2e62` 残留。P8 真实 AI 最小复测仍需用户明确批准。

**2026-07-07 v8.5.8 发布同步完成：MVU JSONPatch 协议修复进入发布版 PNG。** 用户已导入 `神秘复苏模拟器发布版` v8.5.7 并进行了数轮真实对话；只读 Chrome DevTools MCP 验证显示，主要问题不是数据库前端写库失败，而是 MVU / TavernHelper 变量层没有消费当前卡提示词要求的 `<UpdateVariable>` 直接 JSON 数组格式。本轮已把开发版/发布版变量输出契约、系统提示词、事件 MVU 规则和对话示例改为 `<UpdateVariable><JSONPatch>[...]</JSONPatch></UpdateVariable>`；`App.vue` 与 `vendor/shujuku-sp-fork/index.js` 已优先解析 nested `<JSONPatch>` 并保留旧 direct-array fallback；`initvar.yaml` 已移除顶层 `stat_data:`，`变量列表.txt` 只保留旧污染兜底。no-AI 运行态验证确认 nested `<JSONPatch>` 可更新 `stat_data.姓名/所在位置/当前灵异事件/行动建议`，旧 direct-array 仍不被当前 MVU 消费；现有旧消息面板 stale 属旧消息下游表现，需新真实回复验证。可见 ``` 代码围栏泄漏已通过修改两个全局 Tavern Regex replacement 处理，原 replacement 备份在 `.tmp-mfrs-regex-backup-20260707.json`，刷新当前 5 条消息后 DOM closing fence 从 2 个降为 0。source commit `971c617` 已 push，bot bundle `454267e [bot] bundle` / tag `v0.0.375` 已生成并验证状态栏 dist 含 nested JSONPatch 解析；`scripts/publish-card.mjs` 已回填 `CDN_REF=454267e`、`releaseVersion=8.5.8`，`publish-card` 已生成发布版 YAML/PNG；发布同步 commit `5b97c78 chore(release): publish mfrs v8.5.8` 已 push。验证通过：YAML `454267e` 7 处、PNG chara/ccv3 合计 `454267e` 14 处、旧 `bbbe6c7`/8.5.7/local 均 0，worldbook gate 383/33/5851，CDN smoke `@454267e` 状态栏/vendor/数据库前端均 200；远端 `@5b97c78` 发布版 YAML/PNG 均 200，含 8.5.8 / `454267e` 且无旧 ref 或本地链接。未触发真实 AI，未发送消息，未点击“立即手动更新”，未调用 `manualUpdate()` / `triggerUpdate()`。

**2026-07-07 新对话恢复快照（v8.5.7 发布完成时的历史快照）：** 当前主线已完成自动剧情/记忆召回并发布到 `v8.5.7`。最新远端 HEAD 为 `0242af8 docs: update mfrs v8.5.7 release progress`，发布同步 commit 为 `27acf1f chore(release): publish mfrs v8.5.7`，bot bundle 为 `bbbe6c7 [bot] bundle`（tag `v0.0.371`），source commit 为 `f67c780 feat(mfrs): add automatic plot memory recall`。当前应导入/分发的角色卡是 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`，版本 `8.5.7`，CDN ref `bbbe6c7`。当时 `main` 与 `origin/main` 一致，tracked 文件干净；唯一剩余本地项是未跟踪截图 `屏幕截图 2026-07-06 235029.png`，默认不提交。当前新任务以本条上方“发布后真实对话问题修复规划”和下方“当前任务清单”为准。

**2026-07-07 v8.5.7 发布同步完成：自动剧情/记忆召回进入发布版 PNG。** source commit `f67c780 feat(mfrs): add automatic plot memory recall` 已 push；bot bundle `bbbe6c7 [bot] bundle` 已生成并打 tag `v0.0.371`，dist 数据库前端已确认含 `mfrs_auto_plot_memory_recall`、`自动剧情记忆召回`、`toggle-auto-plot`、`toggle-auto-memory`。`scripts/publish-card.mjs` 已回填 `CDN_REF=bbbe6c7`、`releaseVersion=8.5.7`，`pnpm run publish-card -- 神秘复苏模拟器发布版` 已重新生成发布版 YAML/PNG；发布同步 commit `27acf1f chore(release): publish mfrs v8.5.7` 已 push。验证通过：`node --check scripts/publish-card.mjs`、`git diff --check`、`pnpm verify:mfrs-frontend`、发布版 worldbook gate 383/33/5851、发布版 YAML version 8.5.7 + `bbbe6c7`×7 + 0 旧 `573807b`/8.5.6/localhost/127.0.0.1、发布版 PNG `chara`/`ccv3` 各 `bbbe6c7`×7 + 8.5.7×1 + 0 旧 `573807b`/8.5.6/localhost/127.0.0.1、CDN smoke `@bbbe6c7` 数据库前端 200 且含自动召回 marker，固定状态栏 200；推送后远端 `@27acf1f` 发布版 YAML/PNG 均 200，含 8.5.7 / `bbbe6c7` 且无旧 ref 或本地链接。未触发真实 AI，未发送消息，未点击“立即手动更新”，未调用 `manualUpdate()` / `triggerUpdate()`；未跟踪截图 `屏幕截图 2026-07-06 235029.png` 不提交。
...
**当前版本：**
- 最新发布包为 v8.5.8：MVU JSONPatch 协议修复已进入发布版；source `971c617`，bot bundle `454267e`（tag `v0.0.375`）作为 CDN_REF；releaseVersion=8.5.8；发布版 YAML/PNG 已生成并通过本地与 CDN smoke，发布同步 commit `5b97c78` 已 push。
...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（当前已发布版本 8.5.11，CDN `@15c2feb`，cache `mvu-v8511`）
...
**下次恢复入口（当前）：** ✅ **v8.5.11 已完成并推送（MVU 写回持久化 + 消息内面板中文行动建议渲染收口）**。新对话先读本文件顶部“当前状态”、`PROJECT_FLOW.md`、`progress.md` 顶部“v8.5.11 远端发布 smoke 完成”和“DevTools 真页验证 v8.5.11 当前导入卡”、`findings.md` 顶部“v8.5.11 当前导入卡已验证”。当前发布 PNG 为 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`，版本 `8.5.11`，CDN `@15c2feb`，发布同步 commit `93fe875`；当前任务已收口，无当前未决开发项；`.tmp-mfrs-regex-backup-20260707.json` 与截图文件不属于提交内容。
...
**工作区状态：** 当前主线已包含 v8.5.11 source `157917a`、bot bundle `15c2feb`、发布同步 `93fe875`；远端 YAML/PNG smoke 已通过。当前仅有本地参考文件 `.tmp-mfrs-regex-backup-20260707.json` 和未跟踪截图 `屏幕截图 2026-07-06 235029.png`，默认不提交；后续如有新任务仍需精确 staging。
...
**当前任务：v8.5.9/v8.5.10 真实对话问题收口（complete）。** v8.5.10 已修复 MVU message variables 写回/持久化；v8.5.11 已修复消息内面板读取旧字段导致的 `未知行动` 显示。当前导入卡与远端发布版均已验证通过。

**P0 复核运行态入口与失败样本（complete）**
1. 只读复核当前真页角色、卡体版本、请求样本和消息变量快照，固定基线：`reqid=281` 缺完整输出契约、`reqid=317` 有契约但模型仍输出旧 direct-array、message/root variables 仍初始、数据库仪表盘镜像已更新。
2. 记录现有真实回复的原始 `<UpdateVariable>` 样本，保留 `op:"add"`、direct-array、nested `<JSONPatch>` 三类最小样本，后续测试全部复用这些样本，不重新触发真实 AI。
3. 边界：本阶段不发送消息、不点击“立即手动更新”、不调用 `manualUpdate()` / `triggerUpdate()`。

**P1 修 MVU hotfix 调用链（complete）**
1. 修 `src/神秘复苏模拟器/脚本/hotfix-generation-ended-listeners/index.ts`：不要再调用 `Mvu.parseMessage(lastMessageIndex,{})`；改为取最新 AI 消息文本作为第一个参数，并把当前旧 MVU 数据作为第二个参数传入。
2. 确认写回路径使用运行态实际 API：解析后必须把更新后的 MVU 数据写回对应最新消息变量，而不是只拿到返回值后丢弃。
3. 增加防御：找不到最新消息、`message.mes` 为空、`window.Mvu` 不存在或 parse 失败时只记录低噪声日志，不破坏数据库 fallback 路径。

**P2 旧协议兼容归一化（complete）**
1. 在 hotfix 解析前增加归一化：把旧 `<UpdateVariable>[...]</UpdateVariable>` direct-array 转成 `<UpdateVariable><JSONPatch>[...]</JSONPatch></UpdateVariable>`，再交给 `Mvu.parseMessage()`。
2. 支持模型误用的 `op:"add"`：对当前 MVU 可安全表达的数组追加语义转换为 `op:"insert"`；无法安全转换的 patch 记录并跳过，不让整批变量更新失败。
3. 保持 nested `<JSONPatch>` 原样通过，避免二次包裹；旧 direct-array fallback 只作为兼容层，不重新写进提示词主契约。

**P3 修首轮完整输出契约注入（complete）**
1. 排查首轮生成路径为什么没有携带完整 `变量输出格式` / `update_output_contract`：重点看世界书激活策略、开局/自动召回注入顺序、系统提示词和发布版条目深度。
2. 让首轮真实生成也稳定注入完整 nested `<JSONPatch>` 输出契约，不能只出现弱 `<UpdateVariable>` / `<JSONPatch>` 片段。
3. 强化提示词：明确禁止 direct-array、禁止 `op:"add"`，数组追加统一要求 `insert`；同时保留“不要代码块、不要解释文本、路径以 `stat_data` 为根”的约束。

**P4 修真实 vendor import 链路（complete）**
1. 修 `src/神秘复苏模拟器/脚本/数据库/index.ts`、`src/神秘复苏模拟器/脚本/数据库前端/index.ts` 中硬编码旧 vendor `@52b2e62/vendor/shujuku-sp-fork/index.js` 的问题，避免发布包 loader 拉回旧 vendor。
2. 构建后检查 `dist/神秘复苏模拟器/脚本/数据库/index.js` 和数据库前端 dist 不再含旧 `@52b2e62`，并确认真实 loader import 链路使用当前发布 bundle ref。
3. 调整 CDN smoke：不仅直接 smoke `@<bundle>/vendor/...`，还要 smoke 发布卡实际会加载的 loader，再从 loader 内容检查 vendor import ref。

**P5 回归脚本补强（complete）**
1. 给 hotfix / MVU 解析链补最小回归：message text + oldData 输入后，nested `<JSONPatch>`、旧 direct-array、旧 direct-array + `op:"add"` 都能得到预期 `stat_data`。
2. 增加首轮契约静态检查：发布版世界书/系统提示词必须含完整 `<UpdateVariable><JSONPatch>` 骨架，且不再出现“direct array inside `<UpdateVariable>`”旧要求。
3. 增加 import 链路检查：source 和 dist 中不能残留旧 `@52b2e62` vendor hardcode。

**P6 no-AI 真页验证（complete）**
1. 用 Chrome DevTools MCP 或 `scripts/cdp-evaluate.mjs` 只读/本地样本验证 `Mvu.parseMessage(normalizedMessage, oldData)` 能更新 `姓名/风险值/事件代号/鬼域状态/行动建议`。
2. 在真页不触发 AI 的前提下验证消息内面板读取的是更新后的 message variables，而底部数据库仪表盘与 message variables 不再分裂。
3. 复核 console/network 没有新增错误；仍不调用 `manualUpdate()` / `triggerUpdate()`。

**P7 发布链路（complete）**
1. ✅ 静态检查：`node --check scripts\publish-card.mjs`、发布版 worldbook gate、YAML/PNG 元数据、bot dist marker、bundle CDN smoke、`git diff --check` 均通过；P0-P6 阶段已通过 `verify:mfrs-mvu-hotfix`、输出清洗回归、`verify:mfrs-frontend` 和 `pnpm build`。
2. ✅ 精确 source commit/push：`5f38953 fix(mfrs): harden mvu hotfix protocol handling`；bot bundle `e36f8aa [bot] bundle` / tag `v0.0.378` 已验证含 hotfix normalizer、`MysteryMessagePanel.refreshMessage`、runtime URL wrapper 和同 ref vendor 链，无 `file:///` / `@main/` / `@52b2e62`。
3. ✅ 发布同步：`scripts/publish-card.mjs` 已回填 `CDN_REF=e36f8aa`、`CDN_CACHE_VERSION=phase164-4-0-final-baseline-6-28-p5-4-hotfix13-mvu-v859`、`releaseVersion=8.5.9`；`pnpm run publish-card -- 神秘复苏模拟器发布版` 已生成发布版 YAML/PNG，发布同步 commit `ec6b64a chore(release): publish mfrs v8.5.9` 已 push，远端 `@ec6b64a` YAML/PNG smoke 通过。

**P8 用户真实对话复测（complete / failed）**
1. ✅ 用户已导入 v8.5.9 发布版并完成数轮真实对话；本轮只读 Chrome DevTools MCP 验证，未发送新消息、未点击“立即手动更新”、未调用 `manualUpdate()` / `triggerUpdate()`。
2. ❌ 复测未通过：#2/#4 raw protocol 中 nested `<JSONPatch>` 有效，hotfix 也在 console 声称通过 `Mvu.replaceMvuData` 写回，但 `chat[i].variables[0].stat_data`、`TavernHelper.getVariables({ type:'chat' })` 和 `.mfrs-msg-panel` 仍保持初始值。
3. 当前最早不确定点收敛到 P1 写回层：需要查 `Mvu.replaceMvuData` 的参数语义、返回值和真实持久化目标；必要时改为直接写入 `chat[messageIndex].variables[0]` / TavernHelper 正确 setter，并确保刷新与保存链路读取同一份 message variables。

**P9 verified writeback 修复（source complete / pending release）**
1. ✅ 真页探针确认：v8.5.9 的协议、raw 保存、`parseMessage`、`replaceMvuData`、消息内面板刷新 API 均已生效；失败缺口是 hotfix 单次写回无读后校验、无延迟重试、无安装恢复补写。
2. ✅ 已实现 `writeMvuDataWithVerification()`：`Mvu.replaceMvuData` 后立即读回比对 `stat_data`，不一致时通过 `assignMessageVariablesDirectly()` 写入 `chat[messageIndex].variables[swipe_id]`。
3. ✅ `GENERATION_ENDED` 后新增 250ms / 1000ms / 2500ms 延迟重试；安装成功后新增 `recoverRecentRawProtocolMessages()`，扫描最近 12 条 AI 消息 raw protocol 并补写。
4. ✅ 本地构建 hotfix bundle 临时注入当前真页后，恢复扫描已修复 #2/#4 message variables 与 `.mfrs-msg-panel`，未触发真实 AI 或数据库手动更新。
5. ✅ 验证通过：`node --check protocol-normalizer.js`、`node --check verify-mfrs-mvu-hotfix-regressions.mjs`、`pnpm verify:mfrs-mvu-hotfix`、`node scripts/verify-output-cleaning-regressions.mjs`、`pnpm verify:mfrs-frontend`、`git diff --check`、`pnpm build`、构建后 `pnpm verify:mfrs-mvu-hotfix`。
6. ✅ v8.5.10 发布同步已推送并远端 smoke 通过；后续用户真实对话复测证明写回/持久化已生效。
7. ✅ v8.5.11 已修复消息内面板 `未知行动` 显示，发布同步 `93fe875` 已推送，远端 YAML/PNG smoke 通过；当前导入卡 DevTools 验证行动建议已正确显示 `思路` 与风险/收益摘要。

**当前任务：发布后真实对话问题修复（任务 1-5 已实施，目标版本候选 v8.5.8；待 P5 发布链路）。**

**P0 协议根因修复：MVU 输出格式**
1. ✅ 修改开发版与发布版变量输出规则：`src/神秘复苏模拟器/世界书/变量/变量输出格式.yaml`、`src/神秘复苏模拟器发布版/世界书/变量/变量输出格式.yaml`，把“直接数组 inside `<UpdateVariable>`”改成实际 MVU 可消费的 `<UpdateVariable><JSONPatch>[...]</JSONPatch></UpdateVariable>`。
2. ✅ 同步检查系统提示词、世界书规则和对话示例中对 `<UpdateVariable>` 的描述，避免继续暗示直接数组格式；保留“不要代码块、不要解释文本、路径以 `stat_data` 根字段为准”的约束。
3. ✅ 更新 `scripts/verify-output-cleaning-regressions.mjs`：删除“raw UpdateVariable 不应包含 `<JSONPatch>`”的断言，改为断言 nested `<JSONPatch>` 是首选格式，并确保显示层仍隐藏 `<UpdateVariable>` 和 `<JSONPatch>`。

**P1 兼容解析与回归保护**
1. ✅ 更新 `src/神秘复苏模拟器/界面/状态栏/App.vue` 的 `parseUpdateVariablePatchArray`，优先解析 `<JSONPatch>` 包裹的数组，同时保留旧直接数组 fallback，避免旧聊天记录或旧卡失效。
2. ✅ 更新 `vendor/shujuku-sp-fork/index.js` 里 `parseUpdateVariablePatchArray_ACU`、`repairMfrsUpdateVariableBlock_ACU` 和相关行动建议提取逻辑，使 vendor 能识别 nested `<JSONPatch>`，并且不要把新协议重写回错误的直接数组格式。
3. ✅ 补充/调整静态回归脚本，覆盖三类样本：nested `<JSONPatch>` 正常更新、旧直接数组兼容、显示输出不泄漏协议块。

**P2 初始化变量结构修复**
1. ✅ 修正 `src/神秘复苏模拟器/世界书/变量/initvar.yaml` 与发布版同名文件：移除顶层 `stat_data:` 包裹，让 YAML root 与 `src/神秘复苏模拟器/schema.ts` 的 `Schema` 根字段一致。
2. ✅ 复查 `src/神秘复苏模拟器/世界书/变量/变量列表.txt` 与发布版同名文件中 `_.omit(rawStatData, 'stat_data')` 的历史 workaround；root 修复后改为兼容保护，避免掩盖真实变量结构。
3. ✅ 用只读运行态或本地最小样本验证初始化后不再出现 `variables.stat_data.stat_data`，且旧聊天中若已有嵌套污染，不会被展示模板再次放大。

**P3 下游面板验证**
1. ✅ no-AI 复测 `src/神秘复苏模拟器/脚本/消息内面板/index.ts` 读数链路：现有旧 direct-array 消息没有写入 message variables，面板 stale 符合预期；nested `<JSONPatch>` 已确认可由 MVU 更新 `stat_data`。
2. ✅ 当前无证据指向消息内面板渲染时机或 message_id 绑定问题；真实新回复后若仍 stale，再单独排查。

**P4 可见代码围栏泄漏处理**
1. ✅ 记录现象边界：当前可见 `<p><br><br>```</p>` 来自运行态全局 Tavern Regex，而不是 AI raw 消息或本仓库源码。
2. ✅ 已按优先策略编辑两个全局显示正则，去掉 replacement 外层 ```html / ```；未新增额外清理正则。
3. ✅ 修改全局正则前已备份原 replacement 到 `.tmp-mfrs-regex-backup-20260707.json`；刷新当前 5 条消息后 closing fence 段落从 2 个降为 0，未触发生成。

**P5 验证与发布链路**
1. ✅ 静态验证：`git diff --check`、相关 `node --check`、`pnpm verify:mfrs-frontend`、`scripts/verify-output-cleaning-regressions.mjs`、`pnpm build` 已通过。
2. ✅ MVU 最小验证：nested `<JSONPatch>` 会更新变量；旧 direct array 仍不被当前 MVU 消费，仅作为状态栏/vendor 行动建议解析 fallback 兼容。
3. ✅ 真页 no-AI smoke：确认运行态变量层、消息内面板旧消息 stale 边界和显示清洗；未发送消息、不点“立即手动更新”、不调用 `manualUpdate()` / `triggerUpdate()`。
4. ⏳ 若用户批准真实复测，再做一轮最小真实对话验证 MVU message variables 更新；否则停在 no-AI 证据。
5. ✅ 正式链路：source `971c617` → bot bundle `454267e` / tag `v0.0.375` → 回填 `scripts/publish-card.mjs` 为 `CDN_REF=454267e`、`releaseVersion=8.5.8` → `pnpm run publish-card -- 神秘复苏模拟器发布版` → YAML/PNG/CDN smoke 通过。

**当前任务：发布后真实对话修复已完成并发布同步到 v8.5.8；代码、bot bundle、发布版 YAML/PNG 和 CDN smoke 已收口。**

1. ✅ source commit/push：`971c617 fix(mfrs): align mvu update variable protocol`。
2. ✅ bot bundle：`454267e [bot] bundle` / tag `v0.0.375`，状态栏 dist 含 nested `JSONPatch` 解析逻辑。
3. ✅ 发布同步：`scripts/publish-card.mjs` 回填 `CDN_REF=454267e`、`releaseVersion=8.5.8`；发布版 YAML/PNG 重新生成并以 `5b97c78 chore(release): publish mfrs v8.5.8` push。
4. ✅ 发布包验证：发布版 YAML version 8.5.8 + `454267e` 7 处；发布版 PNG chara/ccv3 合计 `454267e` 14 处、8.5.8 两处；旧 `bbbe6c7` / 8.5.7 / localhost / 127.0.0.1 均为 0。
5. ✅ CDN smoke：`@454267e` 状态栏 200 且含 `UpdateVariable` / `JSON[P]atch`，vendor 200 且含 `extractMfrsUpdateVariableJsonPatchArrayText_ACU` / `<JSONPatch>`，数据库前端 200 且保留自动召回 marker；远端 `@5b97c78` 发布版 YAML/PNG 均 200。

**当前任务：自动剧情/记忆召回已完成并发布同步到 v8.5.7；代码、bot bundle、发布版 YAML/PNG 和远端 CDN 验证均已收口。**
...
6. ✅ 等 bot bundle：`bbbe6c7 [bot] bundle` / tag `v0.0.371` 已生成，`dist/神秘复苏模拟器/脚本/数据库前端/index.js` 已确认含自动召回 marker（`mfrs_auto_plot_memory_recall`、`自动剧情记忆召回`、`toggle-auto-plot` / `toggle-auto-memory`）。
7. ✅ 发布同步：`scripts/publish-card.mjs` 已回填 `CDN_REF=bbbe6c7`、`releaseVersion=8.5.7`，`publish-card` 已重新生成发布版 YAML/PNG；发布同步 commit `27acf1f` 已 push，YAML/PNG/CDN 验证通过。
...
|---|---|---|---|---|
| **`v8.5.12`** | **消息内面板最终渲染兜底** — 修复第二轮/后续回复变量已写入但消息内状态面板缺失：SillyTavern 生成结束后可能继续替换最新 `.mes_text`，且事件派发链中其它监听器异常会影响单次 200ms 刷新。本版增加 `GENERATION_ENDED/GENERATION_STOPPED` 后多段延迟刷新，并用 `MutationObserver` 监听最终 `.mes/.mes_text` 替换后补跑 `processAllMessages()` | source `6b77048` → bot bundle `8b3ea67`（tag `v0.0.387`）；publish-card CDN_REF=`8b3ea67`/releaseVersion=`8.5.12`/cache=`phase164-4-0-final-baseline-6-28-p5-4-hotfix13-mvu-v8512`；YAML 8.5.12 + `8b3ea67`×7 + `mvu-v8512`×8 且旧 ref/local 0；PNG chara/ccv3 各 8.5.12 + `8b3ea67`×7 + `mvu-v8512`×8 且旧 ref/local 0；worldbook gate 383/33/5851；CDN smoke `@8b3ea67` 消息内面板/hotfix/数据库 loader 均 200 | `@8b3ea67` / `phase164-4-0-final-baseline-6-28-p5-4-hotfix13-mvu-v8512` / tag `v0.0.387` | **发布同步待提交/push** |
