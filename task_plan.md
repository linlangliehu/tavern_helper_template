...
**2026-07-07 v8.5.10 写回失败根因收口：之前协议/解析修复已生效，verified writeback + persistence 候选待发布。** 本轮用 CDP 真页探针和源码复核确认当前失败不在发布包、vendor 链、nested `<JSONPatch>`、raw protocol 保存、`Mvu.parseMessage()` 或 `Mvu.replaceMvuData()` API 本身：#2/#4 raw protocol 可解析出正确 `stat_data`，可逆手动写回能让 `chat[4].variables[0]` 与消息内面板显示 `测试 / 敲门鬼事件`，3.5 秒等待和 `setChatMessages([{message_id, mes, extra}])` 都未复现回滚。角色卡变量链路已确认：`initvar.yaml` 顶层对齐 schema；`变量列表.txt` 是只读 EJS，读 `variables.stat_data` 并输出 `<stat_data>`；消息内面板按 DOM `mesid` 读取 `getVariables({type:'message',message_id}).stat_data`；数据库前端只读最新 MVU/数据库镜像，不证明历史 message variables 正确。真正缺口是 v8.5.9 hotfix 单次 `replaceMvuData` 后信任日志，没有写后读回校验、直接变量兜底、事件链稳定后的延迟重试、安装恢复补写；本轮进一步确认真页 `getVariables/updateVariablesWith` 位于 `window.TavernHelper`，且直接兜底写内存后需要 `SillyTavern.getContext().saveChat()` 持久化。已在 `src/神秘复苏模拟器/脚本/hotfix-generation-ended-listeners/index.ts` 实现 `writeMvuDataWithVerification()`、`assignMessageVariablesDirectly()`、250/1000/2500ms 延迟重试、`recoverRecentRawProtocolMessages()`、`TavernHelper?.[key]` fallback、缺失 `variables` 时按 swipe 数组初始化，以及直接兜底后的 `saveChat()`；`scripts/verify-mfrs-mvu-hotfix-regressions.mjs` 已加静态 gate。验证通过：`node --check scripts\verify-mfrs-mvu-hotfix-regressions.mjs`、`pnpm verify:mfrs-mvu-hotfix`、`pnpm build`（仅数据库前端 416 KiB 既有 warning）、构建后 `pnpm verify:mfrs-mvu-hotfix`、`git diff --check`。只读 CDP 复核当前真页 #2/#4 message variables 已为 raw protocol 对应值，运行态 `TavernHelper.getVariables/updateVariablesWith`、`Mvu.*`、`ctx.saveChat` 均存在。未发送消息、未触发真实 AI、未点击“立即手动更新”、未调用 `manualUpdate()` / `triggerUpdate()`。下一步：确认 `dist/**` 仍按构建噪声处理，然后精确 source commit/push，等待 bot bundle，并发布候选 v8.5.10。
...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（当前已发布版本 8.5.9，CDN `@e36f8aa`；当前 source 修复候选为 v8.5.10 verified writeback）
...
**待修 bug：** 当前待修为 v8.5.9 真实对话后 MVU message variables 曾未落盘：协议、raw 保存、nested `<JSONPatch>`、同 ref vendor 链、`Mvu.parseMessage(raw, oldData)` 与 `Mvu.replaceMvuData()` API 已确认生效；失败缺口是 hotfix 单次写回没有读回校验、直接变量兜底、延迟重试、安装恢复补写、TavernHelper fallback 和直接兜底后的 `saveChat()` 持久化。v8.5.10 source 候选已实现上述补齐并通过本地验证，待精确 source commit/push 与发布链路。旧的 window.MFRS 挂载失败、自定义编辑器 `bindItemActions()` 和抽卡相关 bug 均已由 v8.2/v8.3/v7.x 线修复，不作为当前停点。
...
**下次恢复入口（当前）：** ✅ **v8.5.7 已完成并推送（自动剧情/记忆召回自动化进入发布版 PNG）**。新对话先读本文件顶部“2026-07-07 新对话恢复快照”、`PROJECT_FLOW.md`、`progress.md` 顶部“2026-07-07 CST（✅ 任务 7 完成：v8.5.7 发布同步）”、`findings.md` 顶部 PowerShell 中文路径编码经验。当前发布 PNG 为 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`，版本 `8.5.7`，CDN `@bbbe6c7`；任务 1-7 已完成，无当前未决开发项；截图文件不属于提交内容。
...
**工作区状态：** 当前主线已包含 v8.5.7 发布同步 `27acf1f`、planning 收口 `0242af8` 和 bot bundle `bbbe6c7`；`main` 与 `origin/main` 一致，tracked 文件干净。当前仅有未跟踪截图 `屏幕截图 2026-07-06 235029.png`，默认不提交；后续如有新任务仍需精确 staging。
...
**当前任务：v8.5.9 真实对话复测失败收口（P0-P8 已完成；P9 已定位并实现 verified writeback source 修复，下一步走 v8.5.10 发布链路）。**
...
6. ⏳ 剩余：确认 `dist/**` 构建噪声不提交，精确 source commit/push，等待 bot bundle，回填 `scripts/publish-card.mjs` 到候选 v8.5.10，重新生成发布版 YAML/PNG 并做 CDN/PNG smoke。
...
|---|---|---|---|---|
| **`v8.5.10 candidate`** | **MVU verified writeback + persistence 修复候选** — v8.5.9 真实对话复测证明协议、raw 保存、nested `<JSONPatch>`、同 ref vendor 链、`Mvu.parseMessage(raw, oldData)`、`Mvu.replaceMvuData()` 与消息内面板刷新 API 已生效；失败缺口是 hotfix 单次写回后没有读回校验、直接变量兜底、事件链稳定后的延迟重试、安装恢复补写、真实运行态 TavernHelper fallback，以及直接兜底后的 `saveChat()` 持久化。候选修复新增 `writeMvuDataWithVerification()`、`assignMessageVariablesDirectly()`、250/1000/2500ms 重试、`recoverRecentRawProtocolMessages()`、`TavernHelper?.[key]` fallback、缺失 variables 时按 swipe 数组初始化和 `persistDirectMessageVariables()`；本地构建 bundle 临时注入真页后已修复 #2/#4 message variables 与 `.mfrs-msg-panel`，只读 CDP 复核 `ctx.saveChat` 存在 | source pending；当前待提交候选为 `src/神秘复苏模拟器/脚本/hotfix-generation-ended-listeners/index.ts`、`scripts/verify-mfrs-mvu-hotfix-regressions.mjs`、planning 三件套；本地验证已通过 `node --check scripts\verify-mfrs-mvu-hotfix-regressions.mjs`、`pnpm verify:mfrs-mvu-hotfix`、`pnpm build`、构建后 `pnpm verify:mfrs-mvu-hotfix`、`git diff --check` | 沿用 v8.5.9 cache，发布时需等待 bot bundle 后回填新 CDN_REF / releaseVersion=`8.5.10` | **pending source commit / release** |
...
**当前待提交候选（2026-07-07 v8.5.10 verified writeback source 修复）：** 精确提交 `src/神秘复苏模拟器/脚本/hotfix-generation-ended-listeners/index.ts`、`scripts/verify-mfrs-mvu-hotfix-regressions.mjs`、`task_plan.md`、`progress.md`、`findings.md`。本地 `pnpm build` 产生的 `dist/神秘复苏模拟器/脚本/hotfix-generation-ended-listeners/index.js` 可证明 bundle 含修复，但 source commit 默认不提交 `dist/**`，正式 dist 由 GitHub bot bundle Action 在推送后重建；`dist/神秘复苏模拟器/界面/状态栏/index.html` 与 `dist/神秘复苏模拟器/脚本/数据库前端/index.js` 属本地构建噪声，不提交。`.tmp-mfrs-regex-backup-20260707.json` 是全局 Tavern Regex 修改前备份，未跟踪截图 `屏幕截图 2026-07-06 235029.png` 是本地参考图，均不提交，除非用户明确要求保存为项目资产。
