...
+ v8.4.7/v8.4.8 状态栏命令式美化与 parent.document 挂载修复（`d99a5ca`/`302016e`）+ v8.4.9 消息内面板注册接线与两列美化（`3617a1c`）+ v8.5.0 EJS 注入完整 stat_data JSON（`36615f3`）。
...
**工作区状态：** 当前主线已包含 v8.5.0 source commit `36615f3`、bot bundle `787f113` 和发布同步 commit `31b144b`。主工作区仍可能有本地 `dist/**` 构建残留、旧截图或临时文件；正式改动优先使用干净 worktree，精确 staging，不提交无关 dirty。

## 当前任务清单

**当前任务：v8.5.0 EJS stat_data 提示词注入已完成、发布并推送。任务清单 1-6 全部完成。**
...
**当前待办：**
1. 可选：用户重新导入 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 后，做一次真实聊天请求体或 AI 行为观察，确认 AI 不再编造 stat_data 字段路径。
