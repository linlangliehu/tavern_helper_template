...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（当前发布版本 8.5.8，CDN `@454267e`）
...
**待修 bug：** 当前待修为 v8.5.8 真实对话后 MVU message variables 不更新：hotfix 错把 message index 传给 `Mvu.parseMessage()`、旧 direct-array 未归一化、`op:"add"` 未兼容、首轮完整输出契约未注入、运行时 loader 仍可拉旧 vendor `@52b2e62`。旧的 window.MFRS 挂载失败、自定义编辑器 `bindItemActions()` 和抽卡相关 bug 均已由 v8.2/v8.3/v7.x 线修复，不作为当前停点。
...
**当前任务：v8.5.8 真实对话失败二次修复（P0-P6 已完成，P7 发布链路进行中，目标版本候选 v8.5.9）。**
...
**P7 发布链路（in_progress）**
1. 静态检查：`git diff --check`、相关 `node --check`、`pnpm verify:mfrs-frontend`、新增回归脚本、必要时 `pnpm build`。
2. 精确 source commit/push，等待 bot bundle，验证 bot dist 含 hotfix、normalizer、新 vendor import 和新回归 marker。
3. 回填 `scripts/publish-card.mjs` 到新 CDN ref / `releaseVersion=8.5.9`，运行 `pnpm run publish-card -- 神秘复苏模拟器发布版`，验证 YAML/PNG 元数据无旧 ref、本地链接或旧版本残留，再提交发布同步。

**P8 用户批准后的最小真实对话复测（pending）**
1. 只有用户明确批准后才触发一次最小真实 AI 回复；触发前固定角色、版本、请求基线、消息楼层、变量快照和数据库快照。
2. 复测判据：请求含完整输出契约；即使模型继续输出旧 direct-array 或 `op:"add"`，hotfix 仍能归一化并写入 message variables；消息内面板与底部数据库仪表盘一致。
3. 若失败，只分析该一次失败样本，不连续重放；优先回到 P1/P2/P3/P4 中最早不确定点。
...
**当前待提交候选（2026-07-07 v8.5.8 二次修复 source）：** source commit 只提交本轮源码、提示词/世界书契约、验证脚本、`package.json` 和 planning：`src/神秘复苏模拟器/index.yaml`、`src/神秘复苏模拟器/世界书/变量/变量输出格式.yaml`、`src/神秘复苏模拟器/系统提示词/0.txt`、`src/神秘复苏模拟器/脚本/hotfix-generation-ended-listeners/index.ts`、`src/神秘复苏模拟器/脚本/hotfix-generation-ended-listeners/protocol-normalizer.js`、`src/神秘复苏模拟器/脚本/消息内面板/index.ts`、`src/神秘复苏模拟器/脚本/数据库/index.ts`、`src/神秘复苏模拟器/脚本/数据库前端/index.ts`、发布版同步的同名提示词/世界书/index YAML、`scripts/verify-mfrs-mvu-hotfix-regressions.mjs`、`package.json`、`task_plan.md`、`progress.md`、`findings.md`。本地 `dist/**` 与两张 PNG 是构建/同步产物，source commit 不提交；发布版 PNG 等新 bot bundle ref 后由 `publish-card` 重新生成。`.tmp-mfrs-regex-backup-20260707.json` 与未跟踪截图 `屏幕截图 2026-07-06 235029.png` 不提交，除非用户明确要求保存为项目资产。

**当前待提交候选（2026-07-07 v8.5.7 发布同步）：** 本轮发布同步只提交 `scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`、planning 三件套（`task_plan.md`、`progress.md`、`findings.md`）。`dist/**` 已由 bot bundle `bbbe6c7` 进入远端历史，不在发布同步提交中重复提交；未跟踪截图 `屏幕截图 2026-07-06 235029.png` 不要提交，除非用户明确要求保存为项目资产。
