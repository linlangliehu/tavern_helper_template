...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（版本 8.4.4，CDN `@6ee50a7`）
- 开发版源码版本：`2.0`（开发版 yaml 版本号，与发布版独立）；v8.4.4 发布版已生成并推送，用户导入新版 PNG 后生效。
...
+ v8.4.3 CHAT_CHANGED 原生事件回退（`294cc1a`）+ v8.4.4 新建聊天 CHAT_CHANGED 轮询等待数据切换（`491fe43`）。
...
**下次恢复入口更新为：** 读 progress.md 顶部 "v8.4.4 发布：新建聊天 CHAT_CHANGED 轮询等待数据切换" 条目。用户重新导入 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 即可获得 v8.4.4 修复。真页验证已通过（2026-06-30）：用户导入 v8.4.4 后手动多次新建聊天，数据库前端均自动恢复神秘复苏 14 表，未停在默认 8 表。
...
**当前无待提交发布同步文件（2026-06-30 v8.4.4 已 push）：** v8.4.4 发布同步提交为 `92b32bd`，包含 `scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`。源码修复已在 `491fe43`（merge `548e9f0`），dist 已由 bot bundle `6ee50a7` 重建；不要提交主工作区本地 `dist/**` 构建残留。
