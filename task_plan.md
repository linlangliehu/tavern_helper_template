...
**2026-07-09 v8.7.0 实施完成 + 真页视觉 smoke 全绿，等 push/发布。** 5 项决策死指标全部命中 computed style。本轮 source 改动：界面美化 +57 行（新 CSS + base64 LOGO）、消息内面板 -13 行（删旧圆角）。`pnpm build` + `verify:mfrs-mvu-hotfix` + `git diff --check` 全绿。真页视觉 smoke（通过 Chrome DevTools MCP evaluate 把 build 后 CSS 注入 Science Worship 真页 + 创建临时 probe 验证 computed style）5 项决策死指标全部命中。临时 probe + 临时 CSS 已清理，无残留 DOM/文件。source 已 staged，待 commit (含 2 src + 3 planning) + push + 等 bot bundle + publish-card v8.7.0 + 远端 smoke。

**v8.7.0 5 项决策最终结果（全部封闭、全部实施、全部验证）：**
...
**LOGO 处理细节：** 用户提供的原始 PNG `ChatGPT Image 2026年7月9日 13_44_41.png` 1254×1254px / 1768 KB，base64 达 2.35 MB 直接内嵌过重。按用户决策缩小到 144×144px PNG / 31 KB，转 base64 得 41968 字符单行无换行；嵌入到 `界面美化/index.ts` stylesheet 中 `::after` 伪元素 `background-image`。

**Science Worship 真页探测硬证据（Chrome DevTools MCP 实测）：** 参考 iframe `TH-message--4--0` 内 `.icon-left` / `.icon-right` 都是 `<img>` 57×57px，src 为 `data:image/png;base64,...`（~51 KB 和 ~64 KB），用 CSS `animation: spin-cw 10s` / `spin-ccw 10s` 整图旋转；iframe 内 `svgCount: 0`。**v8.7.0 沿用此路线（PNG base64 嵌图 + CSS rotate）零风险，但用 `::after` 伪元素替代 DOM `<img>` 以遵守零 JS 约束。**

**v8.7.0 实施完成状态：**
1. ✅ 读取 LOGO PNG 二进制 → 缩到 144×144 → base64 41968 字符
2. ✅ 新增规则到 `界面美化/index.ts` 的 `#mfrs-horror-theme`：`.mfrs-msg-narrative-wrapper` 双层八角 + `::before` 内层网格光斑 + `::after` 右上角 LOGO base64 36×36 + `@keyframes mfrs-narrative-seal-spin` 10s 顺时针
3. ✅ 删除 `消息内面板/index.ts` line 386-397 旧圆角规则（`wrapNarrativeText()` DOM 创建零改动）
4. ✅ `pnpm build` + `verify:mfrs-mvu-hotfix` + `git diff --check` 全绿
5. ✅ 本地真页视觉 smoke：5 项决策死指标全部命中 computed style（详见 `progress.md` 顶部）
6. ⏳ source commit/push → 等 bot bundle → publish-card v8.7.0 → 远端 smoke（待用户 commit/push 授权）
...
**实现阶段源码改动点（本轮已完成）：**
- `src/神秘复苏模拟器/脚本/界面美化/index.ts`（+57 行）：在 `#mfrs-horror-theme` stylesheet 末尾追加 `.mfrs-msg-narrative-wrapper` 新视觉规则 + `::before` 网格光斑内底 + `::after` LOGO 嵌图 + `@keyframes mfrs-narrative-seal-spin` 10s 顺时针
- `src/神秘复苏模拟器/脚本/消息内面板/index.ts`（-13 行）：删除 line 386-397 旧 `.mfrs-msg-narrative-wrapper` 圆角 6px CSS（避免和新规则冲突）；`wrapNarrativeText()` DOM 创建不动
- stylesheet 隔离思路已落地：正文规则全部进 `#mfrs-horror-theme`，状态面板规则留在 `#mfrs-msg-panel-style`，两 stylesheet 完全独立

**下一恢复入口：** 读本文件顶部 + `findings.md` 顶部「2026-07-09 v8.7.0 决策封闭 + v8.8.0 折叠功能预留」一节 + `progress.md` 顶部最近条目。v8.7.0 source 已 staged 待 commit。**下一步 = commit (含 2 src + 3 planning 精确 staging) + push origin main + 等 bot bundle Action + 拿到 bot bundle commit hash 后跑 publish-card v8.7.0 + 远端 smoke**。v8.8.0 折叠功能作为独立下版任务，findings.md 已完整记录其"全都要"要求和 6 个 bug 风险。
...
**下次恢复入口（当前）：** ⏸ **v8.7.0 决策封闭 + v8.8.0 折叠功能预留（未动代码）**。新对话先读本文件顶部"当前状态""当前任务清单"、`findings.md` 顶部「2026-07-09 v8.7.0 决策封闭 + v8.8.0 折叠功能预留」一节、`progress.md` 顶部最近条目。v8.7.0 的 5 项决策已全部封闭，可直接进入实现阶段，但**进入实现阶段前需要用户明确说"开始改"或"开始实现"**；实现阶段 7 步流水线已列在「当前任务清单」中。v8.8.0 折叠功能作为独立下版任务，findings.md D 节已完整记录其"全都要"要求和 6 个 bug 风险。当前 `origin/main = 852447b chore(release): publish mfrs v8.6.0`，本机 main 已同步。v8.6.0 发布版 PNG 视觉全部生效。`.tmp-mfrs-regex-backup-20260707.json`、`.tmp-chrome-mfrs-validation/`、`.tmp-research/`、`Science Worship 20260628.png`、`屏幕截图 2026-07-06 235029.png` 等参考素材仍不提交，保持原 dirty/untracked 状态。
...
**v8.7.0 实施完成 + 真页视觉 smoke 全绿，等 push/发布。本轮已完成 5/5 决策 + 7 步流水线步骤 1-5 + 7（部分）；待步骤 6 commit/push + bot bundle + publish-card + 远端 smoke。**

### 已封决策（5/5）+ 已实施（5/5）+ 已验证（5/5）
1. ✅ 外框视觉：(a) 双层八角 wrapper —— 外红描边 `#b23a32` + 内黑底 `#080404` + 红辉光
2. ✅ 切角尺寸：(i) 10px 八角切角
3. ✅ LOGO 内容：方案 (i) **base64 PNG 嵌图** `<img>` + CSS rotate（沿用 Science Worship 真页硬证据路线）
4. ✅ 旋转方向：(p) 顺时针 10s `@keyframes mfrs-narrative-seal-spin`
5. ✅ 折叠功能：**推迟到 v8.8.0**（v8.7.0 不含，降低 bug 风险）

### v8.7.0 实施完成状态（7 步流水线 5.5/7 待 push）

1. ✅ 读取 LOGO PNG 二进制 → 缩到 144×144 PNG / 31 KB → base64 41968 字符单行无换行
2. ✅ 新增规则到 `src/神秘复苏模拟器/脚本/界面美化/index.ts` 的 `#mfrs-horror-theme` stylesheet（+57 行）：`.mfrs-msg-narrative-wrapper` 双层八角 10px clip-path + `::before` 网格光斑内底 + `::after` LOGO base64 36×36 + `@keyframes mfrs-narrative-seal-spin` 10s 顺时针
3. ✅ 删除 `src/神秘复苏模拟器/脚本/消息内面板/index.ts` line 386-397 旧圆角规则（-13 行，`wrapNarrativeText()` DOM 创建零改动）
4. ✅ `pnpm build` webpack compiled successfully in 5042 ms + `pnpm verify:mfrs-mvu-hotfix` 通过 + `git diff --check` clean
5. ✅ 本地真页视觉 smoke：通过 Chrome DevTools MCP evaluate 把 build 后 CSS 注入 Science Worship 真页 + 创建临时 probe 验证 computed style，5 项决策死指标全部命中（详见 `progress.md` 顶部。截图保存为 `.tmp-v870-smoke.png`，模型不支持识图）
6. ⏳ **source commit/push → 等 bot bundle → publish-card v8.7.0 → 远端 smoke（source 已 staged 待用户 commit/push 授权）**
   - 当前进度：source 2 个文件 (`界面美化/index.ts` + `消息内面板/index.ts`) + 3 个 planning (task_plan/progress/findings) 已准备好精确 staging
   - 下一步：commit 含这 5 个文件 → push origin main → 等 GitHub bundle Action 自动跑 `rm -rf dist && pnpm install && pnpm build` 重建 dist + 打新 tag → 用 bot bundle 的 commit hash 作为 `CDN_REF` 跑 `pnpm run publish-card -- 神秘复苏模拟器发布版` 同步发布版 YAML+PNG 到 v8.7.0
7. ⏳ 同步更新 planning 文件（本轮已更新 task_plan + progress；findings 顶部条目保持不变，等 push 完成后追加发布同步条目）
