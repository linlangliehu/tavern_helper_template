...
- 开发版源码版本：`2.0`（开发版 yaml 版本号，与发布版独立）；v8.4 发布版已生成并推送，用户导入新版 PNG 后生效。
...
**下次恢复入口：** 读 progress.md 顶部 "v8.4 发布版同步完成并推送：正文摘要 + 数据库前端交互迁移" 条目。用户重新导入 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 即可获得新输出契约和数据库前端交互。

**工作区状态：** 当前主线已包含 v8.4 source commit `fb5127a`、bot bundle `065e519` 和发布同步 commit `4a2ab27`。仅剩无关 dirty：`.claude/worktrees/*`、`.tmp-jerryzmtz-my-tavern-scripts/` — 不要提交。

## 当前任务清单

**当前任务：v8.4 正文摘要 + 数据库前端交互迁移已发布并推送。**
