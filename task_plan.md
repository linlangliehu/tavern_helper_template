...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（版本 8.4.6，CDN `@ec3a312`）
- 开发版源码版本：`2.0`（开发版 yaml 版本号，与发布版独立）；v8.4.6 发布版已生成并推送，用户导入新版 PNG 后生效。
...
**下次恢复入口更新为：** ✅ **v8.4.6 已解决，v8.4.7+v8.4.8 完成并真页验证通过**。读 progress.md 顶部 "v8.4.7+v8.4.8 完成" 条目。当前 origin/main HEAD `dd01cb6`，CDN_REF=`bfef412`/releaseVersion=8.4.8。用户重新导入 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`(v8.4.8) 即可获得：消息内不再有坏宏文本 + 输入框上方精美可展开固定状态栏(收起紧凑摘要/展开三分区生存·事件·驾驭/风险变色/进度条)。**关键经验(findings.md 顶部两条)**：①酒馆助手「脚本」跑在 TH-script iframe、操作主窗口 DOM 必须用 `window.parent.document` 不能裸 `document`；②状态栏美化是命令式 getVariables，不是声明式正则+宏(`get_message_variable`/`format_message_variable` 显示层不解析)。无未决任务。
