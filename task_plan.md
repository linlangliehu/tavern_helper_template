...
**下次恢复入口更新为：** 读 progress.md 顶部 "v8.4.6 发布：状态栏正则改为轻量纯文字折叠面板" 条目。用户重新导入 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 即可获得 v8.4.6（AI 消息内显示纯文字折叠状态面板）。真页渲染验证待用户导入 v8.4.6 后确认。v8.4.5 真页验证已通过（开局不再误发调查点）。
