...
**当前版本：**
- 最新发布包为 v8.5.3：恢复 `固定状态栏` 脚本注册并完成 P0 真页收口验证，继续使用 bot bundle `80b09a8`（tag `v0.0.358`）作为 CDN_REF；releaseVersion=8.5.3；代码/发布包提交 `669d79a` 已 push 到 origin/main。
...
**当前任务：P1 剧情/记忆召回前端化开发版已完成并通过非 AI 真页 smoke；下一步按需要进入 source 提交 / 等待 bot bundle / 发布版同步。**
...
6. ⏭️ 若要面向发布版生效，下一步走 source 提交 → bot bundle → 回填 `publish-card.mjs` CDN_REF/releaseVersion → 发布版 PNG 同步。
...
**当前无待提交发布同步文件（2026-07-06 v8.5.3 已 push）：** v8.5.3 代码/发布包提交 `669d79a` 已在 origin/main；提交包含 `src/神秘复苏模拟器/index.yaml`、`scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`、`task_plan.md`、`progress.md`、`findings.md`。不要提交主工作区本地 `dist/**` 构建残留、`.tmp-*`、截图、参考卡文件或 `.claude/worktrees/**`。
