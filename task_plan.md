...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（版本 8.4.3，CDN `@99f92ff`）
- 开发版源码版本：`2.0`（开发版 yaml 版本号，与发布版独立）；v8.4.3 发布版已生成并推送，用户导入新版 PNG 后生效。
...
+ v8.4.3 CHAT_CHANGED 原生事件回退（`294cc1a`）。
...
**下次恢复入口更新为：** 读 progress.md 顶部 "v8.4.3 发布：CHAT_CHANGED 原生事件回退" 条目。用户重新导入 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 即可获得 v8.4.3 修复。
...
**当前无待提交发布同步文件（2026-06-30 v8.4.3 已 push）：** v8.4.3 发布同步提交为 `feeaa18`，包含 `scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`。源码修复已在 `294cc1a`，dist 已由 bot bundle `99f92ff` 重建；不要提交主工作区本地 `dist/**` 构建残留。
