...
**2026-07-07 v8.5.12 source 候选：修复第二轮对话缺消息内状态面板。** 当前导入卡 `@15c2feb ... mvu-v8511` 中，#4 第二轮 AI 回复没有 `.mfrs-msg-panel`，但 `chat[4].variables[0].stat_data` 与 `Mvu.getMvuData({type:'message',message_id:4})` 一致，行动建议长度为 4；手动调用 `MysteryMessagePanel.refreshMessage(4)` 后面板立即出现，证明不是 MVU/EJS/字段映射失败，而是消息内面板自动补渲染漏跑。已在 `src/神秘复苏模拟器/脚本/消息内面板/index.ts` 增加 `GENERATION_ENDED/GENERATION_STOPPED` 后多段延迟刷新，以及 `MutationObserver` 监听最终 `.mes/.mes_text` 替换后补跑 `processAllMessages()`；回归脚本已加静态 gate。验证通过：`node --check scripts\verify-mfrs-mvu-hotfix-regressions.mjs`、`pnpm verify:mfrs-mvu-hotfix`、`git diff --check`、`pnpm build`（仅数据库前端 416 KiB 既有 warning）。未发送消息、未触发真实 AI、未点击“立即手动更新”、未调用 `manualUpdate()` / `triggerUpdate()`。下一步：精确 source commit/push，等待 bot bundle 后发布 v8.5.12。
