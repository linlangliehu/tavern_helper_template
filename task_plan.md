...
**2026-06-29 v8.4 发布版同步完成（待最终 push 确认）：** 按用户要求将聊天正文从“剧情 + 大量 MUV/MVU 面板/选项块”改为“剧情 + `【本轮摘要】`”，正文摘要最多 6 行；`<choices>` 与 `<UpdateVariable>` 保留为后台协议并由显示/清洗链路隐藏。神秘复苏数据库前端新增交互按钮：行动建议行可点“选择”填入酒馆输入框，灵异物品行可点“使用”填入使用物品行动文本；旧 `<sp_*>/<mfrs_*>` 文本面板在输出契约、显示正则和生成后清洗三层停用。source commit `fb5127a` 已 push，bot bundle `065e519`（tag `v0.0.318`）已生成；`publish-card.mjs` 已回填 `CDN_REF=065e519`、`releaseVersion=8.4`，发布版 PNG 已生成并验证。验证已通过：`git diff --check`、`node --check v10_2_visualizer.js`、`node scripts/verify-output-cleaning-regressions.mjs`、开发版与发布版 `verify-worldbook-pollution-gate --expect-mfrs-runtime`、`pnpm build`、发布版元数据 8.4 + 7×`@065e519` + 0×旧 ref、CDN smoke 200。仅有既有数据库前端 bundle 347 KiB 体积 warning。
...
**下次恢复入口：** 读 progress.md 顶部 "v8.4 发布版同步完成：正文摘要 + 数据库前端交互迁移" 条目。v8.4 发布版已生成并验证；若最终 push 已完成，用户重新导入 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 即可获得新输出契约和数据库前端交互。

**工作区状态：** 当前主线已包含 v8.4 source commit `fb5127a` 和 bot bundle `065e519`。发布版同步文件已生成，待精确提交并 push；无关 dirty：`.claude/worktrees/*`、`.tmp-jerryzmtz-my-tavern-scripts/` — 不要提交。

## 当前任务清单

**当前进行中：v8.4 正文摘要 + 数据库前端交互迁移。开发版与发布版均已生成验证，待最终提交/推送。**
...
**当前无待提交发布同步文件（2026-06-29 v8.3 已 push）：** v8.3 发布同步提交为 `15936d1`，包含 `scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`、`task_plan.md`、`progress.md`、`findings.md`。源码修复已在 `c7e5699`，dist 已由 bot bundle `3f71015` 重建；不要提交主工作区本地 `dist/**` 构建残留。
