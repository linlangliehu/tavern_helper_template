...
**下次恢复入口（当前）：** ✅ **v8.5.6 发布同步完成，固定状态栏截图内容已移除。** 新对话先读本文件顶部、`PROJECT_FLOW.md`、`progress.md` 顶部“2026-07-06 CST（✅ v8.5.6 发布同步：固定状态栏截图内容已移除）”。当前发布版为 v8.5.6，发布版 PNG 位于 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`；截图文件不属于提交内容。
...
**工作区状态：** 当前主线已包含 v8.5.6 source commit `def6576` 和 bot bundle `573807b`；发布同步文件待本轮提交推送。当前未跟踪截图 `屏幕截图 2026-07-06 235029.png` 不提交；正式改动优先精确 staging。
...
**当前任务：固定状态栏截图内容已移除并发布为 v8.5.6；发布版 PNG 位于 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`。**
...
5. ✅ 精确 source commit/push：`def6576 feat(mfrs): remove fixed status panel` 已 push；未提交 `dist/**` 或截图文件。
6. ✅ bot bundle：`573807b` / tag `v0.0.369` 已生成；dist 固定状态栏不含截图 UI marker，数据库前端仍含总览/召回/一致性 marker。
7. ✅ 发布同步：`publish-card` 已回填 `CDN_REF=573807b` / `releaseVersion=8.5.6`，发布版 PNG/YAML/CDN/元数据验证通过。
...
**当前待提交候选（2026-07-06 v8.5.6 发布同步）：** source commit `def6576` 与 bot bundle `573807b` 已完成；本次发布同步只提交 `scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 以及必要 planning 记录（`task_plan.md`、`progress.md`）。不要提交截图文件 `屏幕截图 2026-07-06 235029.png`，不要提交 `.claude/worktrees/**` 或本地 `dist/**` 噪声。
