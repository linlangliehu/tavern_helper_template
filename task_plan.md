...
**当前版本：**
- 最新发布包为 v8.5.4：P1 剧情/记忆召回前端化进入发布版；source `0acda89`，bot bundle `eef6274`（tag `v0.0.363`）作为 CDN_REF；releaseVersion=8.5.4；发布版 YAML/PNG 已生成并验证。
...
**当前任务：P1 剧情/记忆召回前端化已完成并同步发布版 v8.5.4；下一步进入 P1 数据库前端体验增强或等待用户导入发布版 PNG 验收。**
...
|---|---|---|---|---|
| **`v8.5.4`** | **P1 剧情/记忆召回前端化** — 数据库前端新增“召回”虚拟 tab，覆盖 10 张关键表，支持搜索、健康检查、复制、填入输入框、固定/取消固定、填入全部固定召回；虚拟 tab 不写入真实表格排序 | source `0acda89` → bot bundle `eef6274`（tag `v0.0.363`）→ publish sync 本轮；publish-card CDN_REF=`eef6274`/releaseVersion=`8.5.4`；发布版 YAML 15 处 @eef6274；PNG chara/ccv3 version 8.5.4 + @eef6274×7 + 0 旧 @80b09a8/8.5.3；worldbook gate 383/33/5851；CDN smoke @eef6274 数据库前端 200/382475 且含召回 marker | `@eef6274` / `phase164-4-0-final-baseline-6-28-p5-4-hotfix13` / tag `v0.0.363` | **发布同步验证通过** |
...
**本轮 v8.5.4 发布同步提交范围：** 精确提交 `scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器.png`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`、`task_plan.md`、`progress.md`、`findings.md`。不要提交主工作区本地 `dist/**` 构建残留、`.tmp-*`、截图、参考卡文件或 `.claude/worktrees/**`。P1 source commit `0acda89` 和 bot bundle `eef6274` 已在 origin/main。
