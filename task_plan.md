...
**下次恢复入口（当前）：** ✅ **v8.5.4 已发布，P1 召回面板进入发布版，下一步进入 P1 数据库前端体验增强。** 新对话先读本文件顶部、`PROJECT_FLOW.md`、`progress.md` 顶部“2026-07-06 CST（🧭 新对话恢复快照：v8.5.4 已发布，下一步进入 P1 体验增强）”。当前 origin/main = `a219bf7`，发布同步 `7a997c2`，bot bundle `eef6274`，source `0acda89`。若用户要验收发布版，导入/替换 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 后做非 AI smoke；若用户要继续开发，从 P1 数据库前端体验增强开始。
