...
**下次恢复入口更新为：** ⚠️ **当前有未决任务**——v8.4.6 状态栏正则改造（借鉴 Science_Worship 纯文字面板）**真页验证失败**：宏 `format_message_variable`/`get_message_variable` 在神秘复苏环境不被解析，面板显示原始 `{{...}}`，origin/main 上的 v8.4.6 是坏的。读 progress.md 顶部 "⚠️ v8.4.6 已发布但真页验证失败" 条目。**新对话先问用户三选一再动手**：①回滚 v8.4.6（恢复 `[界面]状态栏` 到禁用 iframe 原状）②方案 B 命令式重做（正则换空容器+脚本 getVariables 填值，复用固定状态栏逻辑，推荐）③放弃消息内面板。技术结论：神秘复苏是"无宏"环境，状态栏美化只能走命令式 getVariables，不能照搬借鉴卡的声明式正则+宏。v8.4.5 及更早真页验证均已通过。
