...
**下次恢复入口更新为：** 读 progress.md 顶部 "v8.4.4 发布：新建聊天 CHAT_CHANGED 轮询等待数据切换" 条目。用户重新导入 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 即可获得 v8.4.4 修复。可选真页验证：在数据库前端已有 14 表的聊天里新建聊天，确认不停在默认 8 表而自动重新导入 14 表。
