...
- **非阻塞缺陷（已记录）**：2 张表损坏（灵异物品、收录规律）属于可选功能，暂时接受，后续有需求再修复 vendor 源码
- **内存清洗未同步（已记录）**：不影响玩家体验，界面显示正常
- **可选步骤 8-11** 可根据用户需求决定是否执行，或直接收口

**已知问题记录：**
- **F6 数据库 2 表损坏**：vendor/shujuku-sp-fork/index.js 表结构初始化 bug，灵异物品和收录规律表头被截断，根本修复需要修改 vendor 源码（约 5 万行），暂不优先处理
- **内存清洗时机问题**：Hotfix 清洗在 `GENERATION_ENDED` 后执行，界面美化脚本已渲染完成导致内存未同步，但玩家看到的界面已清洗，不影响体验
