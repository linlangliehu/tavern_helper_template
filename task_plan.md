...
**2026-07-06 固定状态栏截图内容移除进入发布链路：** 用户确认当前角色卡不需要截图 `屏幕截图 2026-07-06 235029.png` 中的输入框上方固定状态栏内容。本轮已在开发版移除 `固定状态栏` summary/detail UI（死亡风险、复苏程度、状态、位置、阶段、当前灵异事件、驾驭厉鬼、“神秘复苏14表”按钮），同时保留 `mfrs-fixed-status-host` 的 dashboard/frontend 两槽以承载数据库仪表盘和 14 表前端；数据库前端 host 维护逻辑同步删除旧 status slot 残留；`verify:mfrs-frontend` 已加防回归。验证通过：`pnpm build`、`pnpm verify:mfrs-frontend`、`git diff --check`、真页本地 bundle 注入后 summary/detail/status slot 均不存在且 dashboard/frontend 保留。下一步：精确 source commit/push，等待 bot bundle，回填 `publish-card` 生成 v8.5.6 发布版 PNG。不要提交本地 `dist/**` 构建产物或截图文件。
...
**当前版本：**
- 当前开发版候选（未提交/未发布）：P1 数据库前端体验增强 + P2 MVU/数据库一致性 + P2 抽卡系统增强 + P3 工程维护已完成。待最后阶段执行真页非 AI smoke、source commit/push、bot bundle 和发布同步决策。
- 最新发布包为 v8.5.4：P1 剧情/记忆召回前端化进入发布版；source `0acda89`，bot bundle `eef6274`（tag `v0.0.363`）作为 CDN_REF；releaseVersion=8.5.4；发布同步提交 `7a997c2` 已 push 到 origin/main；发布版 YAML/PNG 已生成并验证。
...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（当前发布版本 8.5.4，CDN `@eef6274`）
...
+ v8.4.7/v8.4.8 状态栏命令式美化与 parent.document 挂载修复（`d99a5ca`/`302016e`）+ v8.4.9 消息内面板注册接线与两列美化（`3617a1c`）+ v8.5.0 EJS 注入完整 stat_data JSON（`36615f3`）+ v8.5.1 抽卡进度按聊天隔离/数据库前端固定位置/双重去重（`5266dc5`）+ v8.5.2 固定状态栏三槽布局（`4f38920`）+ v8.5.3 固定状态栏脚本注册恢复 + v8.5.4 剧情/记忆召回前端化。
...
**下次恢复入口（当前）：** ✅ **P1/P2/P3 开发版已完成，下一步进入最后收口：真页非 AI smoke → source commit/push → bot bundle → 发布同步。** 新对话先读本文件顶部、`PROJECT_FLOW.md`、`progress.md` 顶部“2026-07-06 CST（✅ P3 完成：工程拆分 + 静态验证 + 真页 smoke 清单）”。当前发布版仍为 v8.5.4（发布同步 `7a997c2`、bot bundle `eef6274`、source `0acda89`）；当前未发布开发版改动包括数据库前端源码、P3 配置拆分、验证脚本和 smoke 清单。最后阶段优先按 `mfrs-database-frontend-smoke.md` 做真页非 AI smoke；通过后再精确 source commit/push，等待 bot bundle；如用户要求发布，再回填 `publish-card` 并同步发布版 PNG。
...
**工作区状态：** 当前主线已包含 v8.5.4 发布同步提交 `7a997c2` 和 planning 收口提交 `a219bf7`，origin/main 已同步。主工作区当前已知无关 dirty 为 `.claude/worktrees/agent-aedb9d9f392ecb036`；正式改动优先精确 staging，不提交无关 dirty。
...
**当前任务：移除固定状态栏截图内容并发布 v8.5.6。**

1. ✅ 读取截图并确认不需要的内容：输入框上方固定状态栏 summary/detail、状态数值、事件/厉鬼区块和“神秘复苏14表”按钮。
2. ✅ 开发版源码：固定状态栏脚本不再渲染截图 UI，只保留数据库仪表盘/14 表前端槽位；数据库前端同步清理旧 status slot。
3. ✅ 静态/构建验证：`pnpm build`、`pnpm verify:mfrs-frontend`、`git diff --check` 通过。
4. ✅ 真页非 AI 运行态验证：本地固定状态栏 bundle 注入后 summary/detail/status slot 均移除，dashboard/frontend 保留；未发送消息、未触发真实 AI、未调用 `manualUpdate()` / `triggerUpdate()`。
5. ⏳ 精确 source commit/push：提交固定状态栏源码、数据库前端源码、验证脚本、smoke 清单和 planning 记录；不提交 `dist/**` 或截图文件。
6. ⏳ 等待 bot bundle，验证 dist 固定状态栏不含截图 UI marker，数据库前端仍含总览/召回/一致性 marker。
7. ⏳ 发布同步：回填 `publish-card` 为新 CDN ref / `releaseVersion=8.5.6`，生成并验证发布版 PNG。
...
|---|---|---|---|---|
| **`开发版候选`** | **P1/P2/P3 数据库前端增强与工程维护** — 新增“总览”虚拟 tab（14 表全局搜索、表状态总览、详情预览、复制/填入/打开原表）；新增“一致性”虚拟 tab（`stat_data` vs 数据库关键表差异、状态快照导出、低风险前端修复）；抽卡面板新增聊天 scope、当前聊天数据导出/导入/重置、卡池校验、经济摘要；扩展 `window.MFRS` 抽卡维护 API；P3 拆出 `frontend-config.js`、新增 `verify:mfrs-frontend` 静态验证和 `mfrs-database-frontend-smoke.md` 真页非 AI smoke 清单 | 当前工作区改动：数据库前端源码、配置拆分、P3 验证脚本、smoke 清单和 planning；验证已过 `node --check`、`pnpm verify:mfrs-frontend`、`git diff --check`、`pnpm build`（仅 404 KiB performance warning）；本地 `dist/**` 为构建产物，默认不提交 | 沿用当前发布 marker，尚未生成新 CDN ref | **未提交/未发布；按用户要求待最后阶段做真页 smoke、source 提交、bot bundle、发布同步** |
...
**当前待提交候选（2026-07-06 v8.5.5 发布同步）：** source commit `df48367` 与 bot bundle `843db59` 已完成；本次发布同步只提交 `scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 以及必要 planning 记录（`task_plan.md`、`progress.md`）。不要提交 `.claude/worktrees/**`；本地旧 dist 噪声已临时保存到 stash `temp local dist before bot ff`，不属于发布同步。
**上一发布状态（2026-07-06 v8.5.4 已 push）：** v8.5.4 发布同步提交 `7a997c2` 已在 origin/main；提交包含 `scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器.png`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`、`task_plan.md`、`progress.md`。P1 source commit `0acda89` 和 bot bundle `eef6274` 已在 origin/main。
**上一发布基线：** v8.5.3 代码/发布包提交 `669d79a` 已在 origin/main；发布版使用 `CDN_REF=80b09a8`、releaseVersion=8.5.3。v8.5.4 在其基础上把数据库前端 CDN ref 更新到 `eef6274` 并同步发布版 PNG。
**当前无待提交发布同步文件（2026-06-30 v8.4.6 已 push）：** v8.4.6 为卡本体（开发版 yaml `[界面]状态栏` 正则）改动，source `f5cf6f4`、发布同步 `5dbcd6e`（merge `0976f15`）含 `src/神秘复苏模拟器/index.yaml`、`scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、发布版 PNG。CDN_REF 不变 `ec3a312`（dist 未改），不需 bot bundle。
