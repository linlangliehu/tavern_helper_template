...
**待修 bug：** v8.5.9 真实对话后 MVU message variables 未落盘的根因已定位并在 v8.5.10 发布版中修复：协议、raw 保存、nested `<JSONPatch>`、同 ref vendor 链、`Mvu.parseMessage(raw, oldData)` 与 `Mvu.replaceMvuData()` API 已确认生效；实际缺口是 hotfix 单次写回没有读回校验、直接变量兜底、延迟重试、安装恢复补写、TavernHelper fallback 和直接兜底后的 `saveChat()` 持久化。旧的 window.MFRS 挂载失败、自定义编辑器 `bindItemActions()` 和抽卡相关 bug 均已由 v8.2/v8.3/v7.x 线修复，不作为当前停点。
...
6. ✅ v8.5.10 发布同步已推送并远端 smoke 通过；如需要可由用户重新导入发布版 PNG 做真实对话复测。
...
|---|---|---|---|---|
| **`v8.5.11`** | **消息内面板中文行动建议渲染修复** — v8.5.10 真页验证证明 MVU 写回已成功，最新楼层 `stat_data.行动建议` 已有 `选项/思路/主要风险/预期收益/死亡风险/复苏风险`；“未知行动”来自消息内面板旧渲染只读 `label/text`。本版改为优先读取 `选项/思路`，显示风险/收益摘要，点击按钮填入 `思路` | source `157917a` → bot bundle `15c2feb`（tag `v0.0.384`）；publish-card CDN_REF=`15c2feb`/releaseVersion=`8.5.11`/cache=`phase164-4-0-final-baseline-6-28-p5-4-hotfix13-mvu-v8511`；YAML 8.5.11 + `15c2feb`×7 + `mvu-v8511`×8 且旧 ref/local 0；PNG chara/ccv3 各 8.5.11 + `15c2feb`×7 + `mvu-v8511`×8 且旧 ref/local 0；worldbook gate 383/33/5851；CDN smoke `@15c2feb` 消息内面板/hotfix/数据库 loader 均 200 | `@15c2feb` / `phase164-4-0-final-baseline-6-28-p5-4-hotfix13-mvu-v8511` / tag `v0.0.384` | **发布同步待提交/push** |
...
**当前提交边界（2026-07-07 v8.5.10 远端 smoke planning 记录）：** v8.5.10 source、bot bundle 与发布同步均已推送；本条随远端 smoke docs 提交后无业务待提交。`.tmp-mfrs-regex-backup-20260707.json` 是全局 Tavern Regex 修改前备份，未跟踪截图 `屏幕截图 2026-07-06 235029.png` 是本地参考图，均不提交，除非用户明确要求保存为项目资产。
