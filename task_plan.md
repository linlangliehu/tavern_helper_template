...
**当前版本：**
- **最新发布包为 v8.5.13**：在场人物输出契约修复并经用户真实对话复测通过；纯世界书改动无 dist 变更，CDN_REF 沿用 `8b3ea67`（tag `v0.0.387`）；releaseVersion=8.5.13、cache `mvu-v8513`；单 commit `e6b9ebe` 已 push origin/main；发布版 PNG `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`。
...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（当前已发布版本 8.5.13，CDN `@8b3ea67`，cache `mvu-v8513`）
...
**下次恢复入口（当前）：** 🔧 **v8.5.14 跨角色卡污染清理进行中**（源码改动完成、静态 gate 绿、待全 gate + 真页非 AI smoke + 发布链路）。新对话先读本文件顶部“当前状态”“当前任务清单”“版本变更索引”、`PROJECT_FLOW.md`、`progress.md` 顶部最近 2-3 条、`findings.md` 顶部相关经验。当前 `origin/main = ee794fc [bot] bundle`（在 v8.5.13 source `e6b9ebe` 之后自动重建开发版 PNG），本机 main 已同步。涉及改动文件：`固定状态栏/index.ts`、`数据库前端/index.ts`、`数据库前端/v10_2_visualizer.js`、`scripts/verify-mfrs-mvu-hotfix-regressions.mjs`；CDN_REF 暂仍为 `8b3ea67`，待 source commit/push 后由 bot bundle 替换为新 ref；releaseVersion 暂定 `8.5.14`、cache `mvu-v8514`。`dist/**` 与 `src/神秘复苏模拟器/神秘复苏模拟器.png` 已清理为 HEAD 版本，不手动提交（由 bot 重建）。`.tmp-mfrs-regex-backup-20260707.json` 与未跟踪截图仍不提交。
...
**v8.5.14 跨角色卡污染清理（进行中）：** 切到非神秘复苏卡时主动 cleanup 固定状态栏、数据库前端、ACU visualizer 留在主窗口的 DOM/globals/`__mfrsScriptResourceUrls__`；切回神秘复苏卡时正常重挂。源码改动已完成、回归脚本静态 gate 已通过（`pnpm verify:mfrs-mvu-hotfix` 绿），待全 gate + 真页非 AI smoke + source commit/push -> bot bundle -> publish-card 发布版同步 -> 远端 smoke。

| 文件 | 改动 |
|---|---|
| `src/神秘复苏模拟器/脚本/固定状态栏/index.ts` | 新增 `isMysteryRevivalCardActive()`、`cleanupFixedStatusBar()`、`handleChatChanged()`、`installCleanup()`：非本卡时主动卸载 `mfrs-fixed-status-host`，并响应 `CHAT_CHANGED`/SillyTavern 原生 `event_types.CHAT_CHANGED` 切卡事件 |
| `src/神秘复苏模拟器/脚本/数据库前端/index.ts` | 拆出 `cleanupMfrsDatabaseFrontend()`：清理 `MysteryAcuVisualizer/MFRS/MysteryDatabaseFrontend` 全局、`__mfrsScriptResourceUrls__` markers、ACU styles/DOM；`loadAcuFrontendRuntime()` 改为按需 eager import，主流程不在非本卡时执行；`installDatabaseFrontendCleanup()` 暴露 `__mfrsDatabaseFrontendCleanup__` 选项式清理入口 |
...
| `scripts/verify-mfrs-mvu-hotfix-regressions.mjs` | 新增静态 gate：要求三个源码文件含 `cleanup`/`isMysteryRevivalCardActive`/`__mfrsFixedStatusCleanup__`/`unregisterTableUpdateCallback` marker，且数据库前端不再静态执行 visualizer（改为 runtime) |

**真页非 AI smoke 口径：**
- 切到非神秘复苏卡 → 主窗口 DOM 无 `mfrs-fixed-status-host/.acu-wrapper/.acu-embedded-*`、`window.MFRS/MysteryDatabaseFrontend/MysteryAcuVisualizer === undefined`、`__mfrsScriptResourceUrls__` 中神秘复苏资源键已移除。
- 切回神秘复苏卡 → 固定状态栏 host/数据库前端/消息内面板/抽卡面板/召回 tab/一致性 tab 全部正常挂载与交互。
- 不发送消息、不点"立即手动更新"、不调 `manualUpdate()/triggerUpdate()`。

**已完成任务清单（勿重做）已折叠：** v8.5.13 在场人物输出契约修复已发布并经真实对话复测通过；v8.5.9/v8.5.10 真实对话收口 P0–P9、发布后真实对话修复 P0–P5、自动剧情/记忆召回任务 1–7、数据库前端 P0–P3/总览-一致性-抽卡增强、抽卡系统 9 任务、四优先级改进（v7.6~v8.0）等均已 100% complete。逐项收口详情见下方「版本变更索引」表与 progress.md 对应条目；需要回查旧任务分解时看 progress.md 或 `planning_archive_2026-06/`，不要从旧清单的"待合并/待发布"描述恢复任务。
...
|---|---|---|---|---|
| **`v8.5.14`** | **跨角色卡污染清理** — 切到非神秘复苏卡时主动 `cleanup` 固定状态栏、数据库前端、ACU visualizer 留在主窗口的 DOM/globals/`__mfrsScriptResourceUrls__`；切回本卡时正常重挂。固定状态栏加 `isMysteryRevivalCardActive()` + `handleChatChanged()` + 原生 `CHAT_CHANGED` 订阅；数据库前端拆 `cleanupMfrsDatabaseFrontend()` + `loadAcuFrontendRuntime()` 按需 eagerly import；ACU visualizer 暴露 `cleanup`/`unregisterTableUpdateCallback`；回归脚本加 4 处静态 gate | source 待 commit/push；releaseVersion 拟 `8.5.14`、cache `mvu-v8514`（CDN_REF 待 bot bundle 后回填） | (待定) | **🔧 进行中：源码改动完成，静态 gate 绿，待全 gate + 真页 smoke + 发布链路** |
