...
**2026-07-11「鬼眼封案」Phase 7 已获用户明确授权并开始执行。** 独立任务 worktree 为 `D:\project\tavern_helper_template_ghostseal`、分支 `codex/mfrs-ghostseal`、起始 HEAD `3181948`。Phase 1-6 已完成且全量门禁通过；Phase 7 当前按“精确 source commit → push → 等待 CI `[bot] bundle` → 不可变 CDN ref → publish-card dry-run/正式镜像 → 发布包与真页回归 → planning 收口”顺序推进。禁止 `git add .`，禁止提交本地 dist、截图和探针；发布前 v8.7.4 截图基线保存在 `.tmp-ghostseal-baseline-v874/`，发布版 YAML/PNG 哈希已单独记录。目标发布版本采用 `8.9.0`，避免占用已预留给折叠功能的 `8.8.0`。
...
- [ ] source commit 只含开发版实际源码、新回归脚本和必要 planning；不用 `git add .`，不提交本地 dist、截图、参考素材或既有 dirty。
- [ ] push 后等待 GitHub `[bot] bundle`，用 `git show <bot-sha>:dist/...` 验证 Logo/面板/档案柜 marker；发布 CDN ref 必须用该不可变 SHA/tag，禁止 `@main`。
- [ ] 先执行 `pnpm run publish-card -- 神秘复苏模拟器发布版 --dry-run`，再正式镜像；人工复核 `robocopy /MIR` 的删除/新增范围。
- [ ] 发布 YAML 和 PNG 的 `chara/ccv3` 必须同时满足：新版本×1、新 ref×7、新 cache×8，旧版本/ref/cache、localhost、127.0.0.1、`@main` 均为0；世界书保持383/33/max5851、正则33、脚本8且顺序不变。
- [ ] 通过 SillyTavern UI 正式导入发布 PNG，重跑 Phase 6 真页门禁和最小可逆 CRUD；用 `exportCurrentData()` 清理并确认测试 token 残留0。
- [ ] 发布前保存 v8.7.4 PNG/YAML/hash基线；source未发布用 `git revert` + 新bundle，已发布则 revert source/release后用新版本/cache重新打包，真页紧急回退只走 SillyTavern UI导入旧PNG，禁止文件覆盖。
