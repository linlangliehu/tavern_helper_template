...
**当前任务：P1/P2/P3 开发版与真页非 AI smoke 已完成；下一步是 source commit/push、等待 bot bundle，并按用户决策进入发布同步。**
...
2. ⏳ 精确提交 source：提交数据库前端源码、`frontend-config.js`、P3 验证脚本、smoke 清单、`package.json` 与必要 planning 记录，不提交本地 `dist/**` 噪声或 `.claude/worktrees/**`。
3. ⏳ push 后等待 bot bundle Action 重建 dist 并打 tag。
4. ⏳ 若用户要求发布，再回填 `scripts/publish-card.mjs` 的 `CDN_REF/releaseVersion`，运行 `publish-card` 同步发布版 PNG 并做发布验证。
...
**当前待提交候选（2026-07-06 P1/P2/P3 开发版，提交前仍需真页非 AI smoke）：** 若用户要求 source 提交，精确提交 `src/神秘复苏模拟器/脚本/数据库前端/v10_2_visualizer.js`、`src/神秘复苏模拟器/脚本/数据库前端/index.ts`、`src/神秘复苏模拟器/脚本/数据库前端/frontend-config.js`、`scripts/verify-mfrs-database-frontend-p3.mjs`、`mfrs-database-frontend-smoke.md`、`package.json` 以及必要 planning 记录（`task_plan.md`、`progress.md`、`findings.md`）。不要提交主工作区本地 `dist/**` 构建残留、`.tmp-*`、截图、参考卡文件或 `.claude/worktrees/**`；`dist/神秘复苏模拟器/脚本/数据库前端/index.js` 与 `dist/神秘复苏模拟器/界面/状态栏/index.html` 当前是本地 build 产物/噪声，source push 后交给 bot bundle Action 重建。若进入发布同步，再按流程修改 `scripts/publish-card.mjs` 并由 `publish-card` 生成发布版 YAML/PNG。
