...
**2026-07-07 v8.5.7 发布同步完成：自动剧情/记忆召回进入发布版 PNG。** source commit `f67c780 feat(mfrs): add automatic plot memory recall` 已 push；bot bundle `bbbe6c7 [bot] bundle` 已生成并打 tag `v0.0.371`，dist 数据库前端已确认含 `mfrs_auto_plot_memory_recall`、`自动剧情记忆召回`、`toggle-auto-plot`、`toggle-auto-memory`。`scripts/publish-card.mjs` 已回填 `CDN_REF=bbbe6c7`、`releaseVersion=8.5.7`，`pnpm run publish-card -- 神秘复苏模拟器发布版` 已重新生成发布版 YAML/PNG。验证通过：`node --check scripts/publish-card.mjs`、`git diff --check`、发布版 worldbook gate 383/33/5851、发布版 YAML version 8.5.7 + `bbbe6c7`×7 + 0 旧 `573807b`/8.5.6/localhost/127.0.0.1、发布版 PNG `chara`/`ccv3` 各 `bbbe6c7`×7 + 8.5.7×1 + 0 旧 `573807b`/8.5.6/localhost/127.0.0.1、CDN smoke `@bbbe6c7` 数据库前端 200 且含自动召回 marker，固定状态栏 200。未触发真实 AI，未发送消息，未点击“立即手动更新”，未调用 `manualUpdate()` / `triggerUpdate()`；未跟踪截图 `屏幕截图 2026-07-06 235029.png` 不提交。
...
**当前任务：自动剧情/记忆召回已完成并发布同步到 v8.5.7；下一步是精确提交发布同步结果并 push。**
...
7. ✅ 发布同步：`scripts/publish-card.mjs` 已回填 `CDN_REF=bbbe6c7`、`releaseVersion=8.5.7`，`publish-card` 已重新生成发布版 YAML/PNG；YAML/PNG/CDN 验证通过，待精确提交并 push。
