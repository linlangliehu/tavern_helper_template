...
**2026-07-07 v8.5.10 真页验证成功，新增 v8.5.11 候选修复“未知行动”显示。** 用户重新导入并对话后，本轮用 Chrome DevTools MCP 只读验证当前页：角色 `神秘复苏模拟器发布版`，脚本资源已是 `@c576add ... mvu-v8510`，运行态 `Mvu.*`、`TavernHelper.getVariables/updateVariablesWith`、`ctx.saveChat` 均存在。最新 AI 楼层 #2 的 `chat[2].variables[0].stat_data` 与 `Mvu.getMvuData({type:'message',message_id:2})` 一致，已写入 `姓名=次卧室`、`身份=初级驭鬼者`、`所在位置=大昌市第七中学教室`、`事件代号=七中灵异事件`、`行动建议.length=4`，证明 v8.5.10 的 MVU 写回与持久化修复已生效。“未知行动”不是 MVU 写回问题，而是 `消息内面板` 显示层仍按旧字段 `label/text` 读取行动建议；当前变量/检定建议结构使用中文字段 `选项/思路/主要风险/预期收益/死亡风险/复苏风险`。已在 `src/神秘复苏模拟器/脚本/消息内面板/index.ts` 修复为优先读取 `选项/思路` 并显示风险/收益摘要，`scripts/verify-mfrs-mvu-hotfix-regressions.mjs` 已加静态 gate。验证通过：`node --check scripts\verify-mfrs-mvu-hotfix-regressions.mjs`、`pnpm verify:mfrs-mvu-hotfix`、`git diff --check`、`pnpm build`（仅数据库前端 416 KiB 既有 warning）。下一步：精确 source commit/push，等待 bot bundle，并发布 v8.5.11。
