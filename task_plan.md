...
**当前版本：**
- 最新本地发布包为 v8.5.3：恢复 `固定状态栏` 脚本注册并完成 P0 真页收口验证，继续使用 bot bundle `80b09a8`（tag `v0.0.358`）作为 CDN_REF；releaseVersion=8.5.3。待提交/推送的文件见「需要提交的文件」。
...
- 发布版 PNG：`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`（当前本地版本 8.5.3，CDN `@80b09a8`）
...
+ v8.4.7/v8.4.8 状态栏命令式美化与 parent.document 挂载修复（`d99a5ca`/`302016e`）+ v8.4.9 消息内面板注册接线与两列美化（`3617a1c`）+ v8.5.0 EJS 注入完整 stat_data JSON（`36615f3`）+ v8.5.1 抽卡进度按聊天隔离/数据库前端固定位置/双重去重（`5266dc5`）+ v8.5.2 固定状态栏三槽布局（`4f38920`）+ v8.5.3 固定状态栏脚本注册恢复（本轮 P0）。
...
**下次恢复入口：** 读 progress.md 顶部 "v8.4.1 热修复发布：保留开局自定义角色面板" 条目。用户重新导入 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png` 即可获得 v8.4.1 修复：正文摘要 + 数据库前端交互迁移仍生效，开局自定义角色表单恢复。
...
**工作区状态：** 当前主线已包含 v8.5.2 source commit `4f38920`、bot bundle `80b09a8` 和发布同步 commit `b568870`。主工作区当前已知无关 dirty 为 `.claude/worktrees/agent-aedb9d9f392ecb036`；正式改动优先精确 staging，不提交无关 dirty。
**下次恢复入口更新为：** ✅ **下一阶段改进清单已建立（v8.5.2 真页收口 + 召回前端化优先）**。新对话先读本文件顶部、`PROJECT_FLOW.md`、`progress.md` 顶部“2026-07-06 CST（🧭 下一阶段改进任务清单已写入 planning）”。若用户要求执行任务，优先从 P0 真页收口验证开始；若用户要求做功能开发，优先从 P1“剧情/记忆召回前端化”开始。

## 当前任务清单

**当前任务：P0 收口验证已完成并生成 v8.5.3 发布包；下一步进入 P1 剧情/记忆召回前端化。**
...
**P1 剧情/记忆召回前端化（下一功能主线）**
1. 在 `神秘复苏数据库前端` 新增“剧情召回 / 记忆召回”面板或 tab。
2. 面板读取并搜索 `事件纪要`、`线索`、`人物`、`地点`、`灵异事件`、`厉鬼档案` 等表。
3. 每条召回结果显示来源表、行号、标题/摘要、标签、是否参与提示词注入。
4. 增加“复制 / 填入输入框 / 固定到本轮提示词”的手动操作。
5. 增加召回健康检查：`AutoCardUpdaterAPI` 可用性、14 表模板状态、`事件纪要` 行数、剧情召回开关、向量召回是否启用。

**P1 数据库前端体验增强**
1. 增加 14 表全局搜索，避免逐表查找。
2. 增加表状态总览：行数、最近更新时间、空表、异常字段。
3. 给关键表增加详情预览：事件、线索、人物、厉鬼、物品可快速展开摘要。
4. 统一“选择 / 使用 / 复制 / 填入输入框”交互，避免不同表行为不一致。
5. 对数据库 API 不存在、模板未加载、写库失败、刷新失败增加明显错误提示。

**P2 MVU / 数据库一致性**
1. 对比 `stat_data` 与数据库关键表，提示“状态栏有但数据库无”或“数据库有但状态栏缺失”的差异。
...
3. 增加卡池校验：缺字段、重复名称、概率异常、奖励无法写库。
4. 增加经济平衡面板：调查点收入来源、消耗、保底进度、残屑产出。

**P3 工程维护**
1. 拆分 `src/神秘复苏模拟器/脚本/数据库前端/v10_2_visualizer.js`，优先拆数据库 UI、抽卡、搜索、写库同步、通用 DOM 工具。
2. 给召回、抽卡隔离、固定三槽布局补静态验证脚本。
3. 建立真页 smoke 清单：导入、打开 14 表、切聊天、抽卡、召回、状态栏重挂载。
4. 若进入正式开发，先只改开发版 `src/神秘复苏模拟器/**`，真页验收后再走发布同步链路。
...
|---|---|---|---|---|
| **`v8.5.3`** | **P0 收口热修复：恢复固定状态栏脚本注册** — v8.5.2 三槽源码可用但发布卡缺少 `固定状态栏` 脚本条目，导致 slot 30 为空；本版恢复 id `...3001` 脚本注册，真页确认 dashboard/frontend/status 三槽自然加载，`pagehide` 只清理状态槽，抽卡聊天隔离回归通过 | 本轮改动：`src/神秘复苏模拟器/index.yaml` + `scripts/publish-card.mjs` releaseVersion 8.5.3 + 发布版 YAML/PNG；继续复用 bot bundle `80b09a8`；发布版 YAML 含固定状态栏 @80b09a8；PNG chara/ccv3 version 8.5.3 + @80b09a8×7 + 0 旧 @88fd7f1/8.5.2；worldbook gate 383/33/5851；CDN fixed-status/database-frontend smoke 200 | `@80b09a8` / `phase164-4-0-final-baseline-6-28-p5-4-hotfix13` | **本地完成，待提交/推送** |
...
**本轮 v8.5.3/P0 收口改动需要提交：** `src/神秘复苏模拟器/index.yaml`（恢复 `固定状态栏` 脚本注册）、`scripts/publish-card.mjs`（releaseVersion 8.5.3）、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`、`task_plan.md`、`progress.md`、`findings.md`。不要提交主工作区本地 `dist/**` 构建残留、`.tmp-*`、截图、参考卡文件或 `.claude/worktrees/**`。
**上一发布基线：** v8.5.2 source `4f38920`、bot bundle `80b09a8`、发布同步 `b568870`（tag `v0.0.359`）已在 origin/main；v8.5.3 继续复用 `80b09a8` 资源，只修复卡本体脚本入口。
