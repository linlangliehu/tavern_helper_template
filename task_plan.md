...
+ v8.4.7/v8.4.8 状态栏命令式美化与 parent.document 挂载修复（`d99a5ca`/`302016e`）+ v8.4.9 消息内面板注册接线与两列美化（`3617a1c`）+ v8.5.0 EJS 注入完整 stat_data JSON（`36615f3`）+ v8.5.1 抽卡进度按聊天隔离/数据库前端固定位置/双重去重（`5266dc5`）。
...
**工作区状态：** 当前主线已包含 v8.5.1 source commit `5266dc5`、bot bundle `88fd7f1` 和发布同步 commit `8a777c2`。主工作区仍可能有本地 `dist/**` 构建残留、旧截图或临时文件；正式改动优先使用干净 worktree，精确 staging，不提交无关 dirty。

## 当前任务清单

**当前任务：v8.5.2 固定状态栏三槽布局已完成、发布并推送。**
...
**当前待办：**
1. 用户侧重新导入 `src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`。
2. 可选真页验证：导入 v8.5.2 后确认输入框上方固定区域顺序为“数据库仪表盘 → 14 表前端 → 状态栏”，且状态栏 `pagehide`/重挂载不破坏数据库前端。
3. 可选回归：沿用 v8.5.1 新建两个聊天验证，确认抽卡调查点仍按聊天隔离。
...
**当前无待提交发布同步文件（2026-07-06 v8.5.2 已 push）：** v8.5.2 source `4f38920`、bot bundle `80b09a8`、发布同步 `b568870`（tag `v0.0.359`）已在 origin/main；发布同步包含 `scripts/publish-card.mjs`、`src/神秘复苏模拟器发布版/index.yaml`、`src/神秘复苏模拟器发布版/神秘复苏模拟器发布版.png`。不要提交主工作区本地 `dist/**` 构建残留、`.tmp-*`、截图、参考卡文件或 `.claude/worktrees/**`。
