...
- **数据库前端 2 表损坏（已知 bug，暂不修复）：** 灵异物品（应 9 列）、收录规律（应 10 列）的表头在运行时被截断为只有 `["row_id"]`。模板 JSON 中定义完整，`sourceData.ddl` 也正确，但 `content[0]` 数组在初始化时被截断。`importMysteryTemplate()` 和 `resetTemplate()` 都无法修复这两张表。厉鬼档案（应 11 列）有相同问题但通过 `resetTemplate()` 成功修复。**根本原因**：vendor/shujuku-sp-fork/index.js 中表结构初始化逻辑存在 bug，某些表的 content 数组在解析时被错误截断。**影响范围**：灵异物品（可选资源，玩家可以不依赖灵异物品通关）、收录规律（特定玩法"鬼拓本/鬼手印"专用，非核心机制）。**临时规避**：这两张表不影响核心游玩流程（行动建议、玩家状态、事件纪要、检定建议、厉鬼档案等核心表全部正常），暂时接受缺陷，后续有需求再修复 vendor 源码。
