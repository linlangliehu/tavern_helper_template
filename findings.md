...
- 开局数据分叉来自欢迎页只提示顶层 `/驾驭厉鬼`，而运行 HUD读取 `/驭鬼者状态/已驾驭厉鬼`；已明确同轮双 replace 与 `厉鬼名称`→`代号` 映射。
- 复苏 100 原规则允许“厉鬼复苏或极端濒死”二选一，未闭合终态；现已固定为独立 `厉鬼复苏` 终态并结束模拟，有效豁免必须同轮落盘且风险降到 100 以下。
- 发布目录重复可导入卡的根因不是遗留文件本身，而是 `publish-card.syncFiles` 每次把开发卡 PNG复制为头像；现保留头像文件名兼容 YAML，但自动移除 `chara/ccv3`。
- 正式发布 PNG只能在新源码 push 后取得 CI bot dist SHA再生成；当前源码、开发 PNG、发布源已修复，旧正式 PNG尚未包含新 bundle。
