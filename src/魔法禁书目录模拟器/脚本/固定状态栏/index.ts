...
const mjrCardNames = new Set(['魔法禁书目录模拟器']);
const mjrCardAvatars = new Set(['魔法禁书目录模拟器.png']);

function isMjrCardIdentity(name?: string | null, avatar?: string | null): boolean {
  if (name) {
    if (mjrCardNames.has(name)) return true;
    // 兼容任何以「魔法禁书目录模拟器 · DEV …」命名的本地开发卡（如手动导入的调试卡）
    if (/^魔法禁书目录模拟器(?:\s*[·•-]\s*|\s+)DEV\b/u.test(name)) return true;
  }
  if (avatar) {
    if (mjrCardAvatars.has(avatar)) return true;
...
function isMjrCardActive() {
  const character = getCurrentCharacter();
  return Boolean(character && isMjrCardIdentity(character.name, character.avatar));
}
